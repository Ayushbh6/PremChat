...
`read`, `search`, and `trace_retrieve` are read-only. Through the governed resource protocol, `read`/`search` cover authorized workspace files plus identity, user profile, generated tool guidance, installed skills, project resources, `.socrates` project memory/notes, repo doctrine, and capability cards. Identity, user profile, generated tool guidance, and installed skills are never writable by main Socrates. Identity/profile proposals use `memory_note`; skill creation/update/import/removal and MCP configuration/check/removal use the always-visible `capability_manager`. Every skill/MCP mutation is approval-gated, exact-source-bound, and delegated to the existing Skill Writer/package-import/MCP owners. Never invent packages, URLs, commands, or credentials, and never use Terminal to bypass this lifecycle.
...
`socrates://project/resources` must resolve through backend project resource records and should be preferred before shell probing when the user asks about uploaded project files under `.socrates/resources/`. Search/list output stays to filenames and bounded metadata until Socrates reads an exact resource.

Chat screenshots/images are different from project resources. They are stored under `<workspace>/.socrates/attachments/`, tracked by `message_attachments`, and referenced in user messages with filename, MIME type, size, and path. The agent prompt teaches Socrates to use native image parts when available, and to reopen known attachment paths with `read` when prior or exact screenshot inspection is needed. Attachments are not surfaced through `socrates://project/resources`. Attachment files may remain after a conversation is deleted; if trace retrieval has no visible provenance, Socrates must not invent the deleted conversation title or message context from the filename alone.

The `edit` tool is the primary model-visible single-file mutation tool. Existing files use one or more exact disjoint replacements in `edits[]`; every match is evaluated against the same freshly read original content, overlaps are rejected, and the file is written atomically once. Whole-file `content` on an existing ordinary workspace file requires explicit `overwrite: true` and is reserved for deliberate rewrites. Durable `.socrates` project memory/notes and repo doctrine are stricter: base resource URIs are read/search only, and every mutation must read and edit one exact section URI. The backend applies the section-local replacement, validates the complete prospective structured document, and atomically persists it; whole-document append/replace authority does not exist. The separate `apply_patch` tool covers multi-hunk or multi-file ordinary workspace patches. Both show a diff or equivalent preview and require approval for ordinary workspace files unless full-access policy explicitly allows the operation. Model-facing inputs never include `dryRun` or content hashes; preview is runtime-owned. Existing-file edits, patches, deletes, and renames require a prior active-turn read, stale hashes are rejected, and another mutation to the same file after success requires a re-read. Governed writable sections and transient work resources may be updated without user approval through the resource authority adapter, but backend-owned sections, identity, user profile, generated tool guidance, and skill files remain protected.
...
`.socrates/` is Socrates-owned memory, runtime, resource, and transient agent-work storage. Free-form plans, task records, `notes/`, and `work/` are transient working conventions rather than durable memory surfaces; create and edit them normally when they help the task, keep them current at meaningful milestones, and condense or remove obsolete scratch material that would mislead a later run. They are not locations for production code, user deliverables, migrations, or permanent tests. Governed `edit` owns exact writable sections in durable project memory/doctrine plus ordinary transient resources while the authority adapter protects base documents, runtime-owned sections, and all skill files. Ordinary "do not edit files", "make no workspace changes", or "review only" instructions protect user workspace artifacts but do not by themselves suppress narrowly scoped backend-owned memory housekeeping. Suppress it when the user semantically includes Socrates memory, project notes, internal state, `.socrates`, or all changes whatsoever.
...
`socrates://project/memory` and `socrates://project/notes` are the main workspace memory resources. Memory stores durable cross-conversation project goals, decisions, constraints, blockers, preferences, changed workflow facts, and handoff facts. Notes store live assistant context, todos, checked files, partial progress, next commands, restart points, and protected backend-owned `runtime_context` and `state_ledger` sections. Base URIs expose full/index reads but are never mutation targets. A write requires an exact URI such as `socrates://project/memory/handoff` or `socrates://project/notes/active_context`; the complete prospective document is validated and atomically written with a section stamp. Project-memory review and mutation occur only when lifecycle evidence identifies a meaningful durable-state change; a workspace mutation by itself is not such a change.

`socrates://project/repo-docs` owns the runtime `.socrates/repo_docs/*.md` doctrine files. They are structured Markdown with stable section ids, backend runtime metadata, and verified mutation stamps. The repo-doc index and file base URIs are read/search only; mutation requires `socrates://project/repo-docs/<file>/<sectionId>`. Root maintainer documentation lives in `context-files/*.md`, not `.socrates/repo_docs/`, so do not confuse it with Socrates runtime-owned docs.

Memory candidate retrieval must stay centralized, automatic, read-only, and surface-aware. Its first pass runs concurrently with goal-candidate retrieval and includes the current capsule when available. After the same-Socrates goal decision, a changed bound goal or empty eligible first pass may trigger exactly one targeted query through that same retrieval service; deterministic selection then filters and reranks exact candidates by authorization, resource scope, bound goal/task, provenance, relevance, and duplication. There is no model-driven Memory Router or second retrieval authority in the target critical path. The same main Socrates that performed the work reconciles important project/repo state inside its normal working loop and verifies any exact mutation; no detached reconciliation call is allowed. Asynchronous Global Memory work owns profile/identity curation. Backend-owned `runtime_context` and `state_ledger` sections are never reconciliation targets.
...
Always-apply rules must be centralized and capped, not scattered through arbitrary docs. The accepted shape is a small global lane in `user_profile.md` named `Global Always-Apply Rules` plus a project lane in workspace project memory named `Project Always-Apply Rules`, each capped at 10 human-readable rules. The backend must attach them with the three bounded identity sections through one per-project stable-prelude snapshot before conversation/user text, not five visible tool calls. Cache reuse is stat-fast-path plus content-hash validation: same-content rewrites and unrelated-section edits retain the snapshot, only a changed standing-section hash rebuilds it, and the in-memory cache must remain bounded. Pre-turn routing must hard-skip standing targets already in the snapshot and deduplicate exact dynamic targets. These rules are for hard behavior constraints only; larger explanations and repo doctrine remain in `.socrates/repo_docs/*`. Current goal/memory/capability context and tool results stay in the dynamic tail. Memory/action ledger messages are forbidden rather than moved elsewhere.
...
The durable `.socrates` architecture has one code-owned authority: `packages/contracts/src/socratesSurfaces.ts`. It defines the nine global/project durable surfaces, paths and aliases, read tools, write owners, load policies, cache classes, and mutation restrictions. Runtime path guards and storage helpers must derive from that registry; prompts and docs must not maintain competing durable path maps. `.socrates/notes` is free-working space for plans, tasks, experiments, temporary scripts, and progress notes; `.socrates/memory` stores important project knowledge and decisions; `.socrates/repo_docs` stores verified repository facts; `memory_note` sends identity/profile/cross-project-memory/skill candidates to the Global Memory Agent. These are used naturally, not ceremonially. The generated model-facing surface map is only a compact projection. Stable model context order is base prompt, compact identity core, global rules, project rules, then the generated surface map; project metadata, retrieval results, runtime facts, attachments, and tool exchanges remain dynamic afterward. Per-batch action/memory ledgers and synthetic steering messages are forbidden.

Use `trace_retrieve` for raw conversation/tool provenance, `socrates://tool-guidance` for generated tool behavior, `socrates://project/memory` and `/notes` for workspace state, `socrates://project/repo-docs` for repository doctrine, `current_time` for current date/time, and `socrates://user/profile` for durable cross-project profile facts.
