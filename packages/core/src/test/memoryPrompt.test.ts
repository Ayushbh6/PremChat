...
describe("memory agent prompt", () => {
  it("defines the backend memory-agent operating contract", () => {
    ...
    expect(memoryAgentBasePrompt).toContain("skills are at `socrates://skills/{builtin|global|project}/{name}`")
  ...
  it("keeps skill installation exclusive to main Socrates", () => {
    expect(memoryCapabilities.get("read")?.permission).toBe("read")
    expect(memoryCapabilities.get("search")?.permission).toBe("read")
    expect(memoryCapabilities.get("capability_manager")).toBeUndefined()
    expect(skillWriterCapabilities.get("read")?.permission).toBe("read")
    expect(skillWriterCapabilities.get("search")?.permission).toBe("read")
    expect(skillWriterCapabilities.get("capability_manager")).toBeUndefined()
  })
})
