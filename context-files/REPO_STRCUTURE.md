...
- Goal and memory candidate retrieval are catalogued automatic services over the shared retrieval foundation. They start concurrently. The selected main Socrates performs the strict zero-tool current/older/new/clarify decision, deterministic code binds the task and selects authorized exact memory, and same-Socrates milestone/final checkpoints own reconciliation. No Goal Router or Memory Router role remains.
