...
- A bounded transient work convention under `.socrates/`: `PLAN.md` for non-trivial implementation plans, `TASKS.md` for the live checklist, and `work/` for disposable scripts, probes, diagnostics, and temporary artifacts. These are not durable memory authorities; production code and permanent tests remain in the repo.
...
- A released no-tool `CompressorAgent` that remains migration debt until lossy conversation compaction is gated by the user's specific pre-dispatch consent; exact sources must remain canonical.
- No production diary read/write/search path and no per-turn main-chat wake-context injection.
- Phase 3 unified pre-turn lifecycle convergence is implemented on `codex/agent-core-rebuild`: Classic and Flow retrieve typed goal/memory candidates concurrently, the selected main Socrates performs the no-tool four-way goal decision, deterministic code selects exact memory, and both views use one exact prepared-context policy. The former Goal Router, Memory Router, `goal_search`, `memory_search`, router settings/prompts, sequential benchmark, and sliced/view-specific context authorities are removed.
...
- Foreground recall runs hybrid goal/memory candidate retrieval concurrently, always supplies the current capsule and latest exact exchange independently of score, then deterministically filters exact memory after goal binding. Main Socrates owns deeper exact inspection and required reconciliation.
...
- Replace released automatic lossy compaction with the explicit pre-dispatch consent contract. Preserve exact canonical messages and whole selected items, state the exact proposed transformation and provider limit, and never infer permission from token pressure.
