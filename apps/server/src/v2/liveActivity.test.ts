...
describe("V2 live activity presentation", () => {
  it("maps tool execution to one bounded user-facing label", () => {
    expect(v2ToolActivity("turn_1", "read", { path: "/workspace/src/traceRetrieveTool.ts" })).toEqual({
      turnId: "turn_1",
      phase: "tool",
      label: "Reading traceRetrieveTool.ts…",
    })
    expect(v2ToolActivity("turn_1", "search", { query: "secret search text" }).label).toBe("Searching the workspace…")
  })

  it("never exposes unknown tool names, raw arguments, or undefined labels", () => {
    const activity = v2ToolActivity("turn_1", "internal_opaque_tool_927", {
  ...
  it("rejects labels outside the shared presentation contract", () => {
    expect(() => createV2LiveActivity("turn_1", "thinking", " ")).toThrow()
    expect(() => createV2LiveActivity("turn_1", "thinking", "x".repeat(121))).toThrow()
  })
})
