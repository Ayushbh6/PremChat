...
`context-files/FLOW_NORTH_STAR.md` is the product-intent authority: Classic and Flow must converge on two projections of one canonical Socrates work state, with goals containing tasks, completion separated from view selection, meaningful follow-ups reopening the same goal, and no replacement Q&A copies merely to switch views. It also owns the target Projects → Goals → Queries Flow navigation, the one-changing-activity-line orb choreography, validated-answer foreground transition, and one shared `AgentRuntime` beneath all model-driven capabilities. `context-files/FLOW_CONVERGENCE_PHASE_0.md` records the machine-checked starting topology; `context-files/FLOW_CONVERGENCE_PHASE_1.md` records the implemented shared runtime and validated main-answer finalization boundary. The isolated experimental V2 Seamless Flow first product cut remains implemented behind its released feature boundary. Its current mechanics, migration constraints, and limitations live in `context-files/V2_FLOW_ARCHITECTURE.md`.
