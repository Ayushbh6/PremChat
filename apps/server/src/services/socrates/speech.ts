...
export const SOCRATES_OPENROUTER_STT_MODELS = [
  "nvidia/parakeet-tdt-0.6b-v3",
  "microsoft/mai-transcribe-1.5",
  "mistralai/voxtral-mini-transcribe",
] as const

export type SocratesOpenRouterSttModel = (typeof SOCRATES_OPENROUTER_STT_MODELS)[number]
export type SocratesWhisperModel = "base.en" | "small.en"
...
const openRouterSttModelSet = new Set<string>(SOCRATES_OPENROUTER_STT_MODELS)
const DEFAULT_OPENROUTER_STT_TIMEOUT_MS = 120_000

export const isAllowedOpenRouterSttModel = (modelId: string): modelId is SocratesOpenRouterSttModel =>
...
export class OpenRouterTranscriber {
  ...
  async transcribe(input: {
    ...
    if (!isAllowedOpenRouterSttModel(input.modelId)) {
      throw new SocratesError("v2_stt_model_not_allowed", "That hosted transcriber is not enabled for Socrates.", {
        details: { modelId: input.modelId, allowedModelIds: SOCRATES_OPENROUTER_STT_MODELS },
        recoverable: true,
      })
    }
...
export class LocalWhisperTranscriber {
  constructor(
    private readonly options: {
      binaryPath: string
      modelPath: (model: SocratesWhisperModel) => string
      timeoutMs?: number
...
export const SOCRATES_SPEECH_PACK_MANIFEST: Record<SpeechPackId, SpeechPackManifestEntry> = {
  "whisper-base.en": {
    id: "whisper-base.en",
    version: "ggml-main",
...
export class SpeechPackManager {
  ...
  constructor(
    ...
    private readonly request: typeof fetch = fetch,
    private readonly manifest: Record<SpeechPackId, SpeechPackManifestEntry> = SOCRATES_SPEECH_PACK_MANIFEST,
  ) {}
