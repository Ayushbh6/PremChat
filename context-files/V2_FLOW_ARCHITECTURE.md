...
This document records the current implemented architecture, migration constraints, and technical mechanics of the experimental Socrates V2 Seamless Flow experience. `FLOW_NORTH_STAR.md` is the product-intent authority for the target Classic/Flow model. Phase 3 replaced the released mirror-based Q&A bridge with canonical task/message identity and reference-based projections; namespaced physical runtime tables remain migration adapters rather than separate semantic state.
