...
- The newest approximately 70k tokens of completed Q/A remain exact by complete-turn boundary after reserving the stable prefix and active turn. Whole exchanges are preserved rather than token-sliced; small fixtures use proportional thresholds.
- Current-turn large tool outputs receive qualifying-only turn-local `R<n>` handles. Socrates may release irrelevant handles with its next otherwise-needed tool call; the release only prunes live model context, never the exact persisted evidence, and automatic compaction does not rewrite the active turn.
