...
- The prepared turn already includes the resolved current goal capsule, latest exact exchange, and selected exact memory. Read project_docs or trace_retrieve only when the task needs deeper project state or exact older evidence; greetings do not require a ceremonial docs call.
...
- When the prepared capsule and latest exact exchange are insufficient for "continue", "where were we", or a broad status question, inspect project_docs notes active_context or exact trace evidence before giving a generic reply. Use user_profile active_context only when the conversation needs global personal context.
...
- Durable-state operating loop:
  1. Load applicable project or repo doctrine when the task actually depends on it; do not read both before every action.
  ...
  3. Use project_docs notes for live information that must survive a wait, restart, or handoff. A successful Terminal or file tool alone does not require memory review.
...
- If verified repo rules, provider behavior, tool behavior, architecture, or contracts materially changed, update the canonical repo_docs section before final unless the fact is transient or already accurate.
...
- Treat .socrates as the agent's flexible working space for free-form plans, task tracking, disposable probes, and one-off helper scripts. Filenames are not prescribed. These are working artifacts, not substitutes for project_docs memory/notes or repo_docs doctrine.
