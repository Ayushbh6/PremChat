...
Before any Classic/Flow lifecycle, goal/task routing, reconciliation, trace-retrieval, focus-ledger, finalization, or cross-view work, read `context-files/REPO_RULES.md`, `context-files/FLOW_NORTH_STAR.md`, and `context-files/UNIFIED_SOCRATES_LIFECYCLE.md`. The unified lifecycle is the detailed target and cleanup authority; phase reports are historical. Its core invariant is one prepared goal/task context and one shared Socrates runtime: Goal Router, read-only pre-turn Memory Router, main work, same-Socrates milestone/final `.socrates` reconciliation, mandatory no-tool structured answer, validation, atomic answer/task/goal commit, then publication. There is no target post-turn Memory Router or mutable main-agent focus-ledger authority.
...
- A first-class no-tool `CompressorAgent` that runs both chat and memory compaction through `AgentRuntime`, mode-specific prompts, explicit empty tool registries, and strict structured output schemas.
- No production diary read/write/search path and no per-turn main-chat wake-context injection.
- A read-only pre-turn Memory Router that selects bounded project/global memory only after the Goal Router has bound the exact canonical goal/task. Main Socrates itself owns milestone and mandatory final `.socrates` reconciliation; no detached post-turn router remains.
...
- Memory Router is a real read-only `MemoryRouterAgent` using `AgentRuntime`, automatic hybrid prefetch over the current user prompt, and a strictly pre-turn `readTargets` contract. The same main Socrates reviews its verified lifecycle evidence at the mandatory reconciliation checkpoint and owns any exact project-doc writes and re-reads.
...
## Flow North Star And Current V2 Implementation

`context-files/FLOW_NORTH_STAR.md` is the product-intent authority: Classic and Flow are two projections of one canonical Socrates work state, with goals containing tasks, completion separated from view selection, meaningful follow-ups reopening the same goal, and no replacement Q&A copies merely to switch views. It also owns Projects → Goals → Queries Flow navigation, the one-changing-activity-line orb choreography, validated-answer foreground transition, and one shared `AgentRuntime` beneath all model-driven capabilities. `context-files/FLOW_CONVERGENCE_PHASE_0.md` records the machine-checked starting topology; Phases 1 through 3 record the shared runtime/finalization, ordered shared routing, and canonical reference-based work identity; `context-files/FLOW_CONVERGENCE_PHASE_4.md` records the implemented navigation and presentation endpoint. The isolated experimental V2 Seamless Flow remains behind its released feature boundary. Its current mechanics, migration constraints, and limitations live in `context-files/V2_FLOW_ARCHITECTURE.md`.
