...
The `edit` tool is the primary model-visible single-file mutation tool. Existing files use one or more exact disjoint replacements in `edits[]`; every match is evaluated against the same freshly read original content, overlaps are rejected, and the file is written atomically once. Whole-file `content` on an existing file requires explicit `overwrite: true` and is reserved for deliberate rewrites. The separate `apply_patch` tool covers multi-hunk or multi-file patches. Both show a diff or equivalent preview and require approval for ordinary workspace files unless full-access policy explicitly allows the operation. Model-facing inputs never include `dryRun` or content hashes; preview is runtime-owned. Existing-file edits, patches, deletes, and renames require a prior active-turn read, stale hashes are rejected, and another mutation to the same file after success requires a re-read. Governed `.socrates` project memory/notes, repo doctrine, and transient work resources may be updated without user approval through the resource authority adapter, but backend-owned sections, identity, user profile, generated tool guidance, and skill files remain protected.
...
`.socrates/` is Socrates-owned memory, runtime, resource, and transient agent-work storage. Free-form plans, task records, and `work/` are transient working conventions rather than durable memory surfaces; create only what helps the task, keep it current at meaningful milestones, and condense or remove obsolete scratch material that would mislead a later run. They are not locations for production code, user deliverables, migrations, or permanent tests. Governed `edit` owns writable project memory/doctrine and transient resources while the authority adapter protects runtime-owned sections and all skill files. Ordinary "do not edit files", "make no workspace changes", or "review only" instructions protect user workspace artifacts but do not by themselves suppress narrowly scoped backend-owned memory housekeeping. Suppress it when the user semantically includes Socrates memory, project notes, internal state, `.socrates`, or all changes whatsoever.
