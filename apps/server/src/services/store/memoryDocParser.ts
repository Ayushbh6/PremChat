...
const sectionDefinitions: Record<MemoryDocType, SectionDefinition[]> = {
  ...
  project_notes: [
    {
      ...
      body: [
        ...
        "    - none",
        "  suggested_virtual_environment: none",
    ...
    { id: "active_context", kind: "context", heading: "Active Context", tags: ["work", "recall"], body: "- Project-local open loops and active recall belong here." },
  ...
  user_profile: [
    ...
    { id: "active_context", kind: "active", heading: "Active Context", tags: ["user", "active"], body: "- No global active user context captured yet." },
    {
      id: "evidence_index",
      kind: "evidence",
...
export const ensureStructuredMemoryDoc = (filePath: string, profile: MemoryDocProfile): void => {
  ...
  try {
    ...
    if (profile.docType !== "identity" && profile.docType !== "user_profile") {
      const repaired = addMissingRequiredSections(current, parsed, profile)
      if (repaired !== current) {
        const repairedIndex = parseMemoryDoc(repaired, profile)
        if (isValidStructuredMemoryDoc(repairedIndex, profile.docType, repaired)) {
...
const addMissingRequiredSections = (content: string, index: MemoryDocIndex, profile: MemoryDocProfile): string => {
  const definitions = sectionDefinitions[profile.docType]
  const required = new Set(memoryDocRequiredSections[profile.docType])
  const existing = new Set(index.sections.map((section) => section.sectionId))
  let next = content.endsWith("\n") ? content : `${content}\n`
  for (const definition of definitions) {
    if (!required.has(definition.id) || existing.has(definition.id)) {
      continue
    }
    const rendered = renderSection(definition).trimEnd()
    const previous = [...definitions.slice(0, definitions.indexOf(definition))]
      .reverse()
      .find((candidate) => existing.has(candidate.id))
    if (previous) {
      const pattern = new RegExp(`(<!--\\s*socrates:section\\s+[^>]*id="${escapeRegExp(previous.id)}"[^>]*-->[\\s\\S]*?<!--\\s*/socrates:section\\s*-->)`)
...
const sectionIdForMigratedEntry = (docType: "identity" | "user_profile", id: string | undefined, heading: string | undefined): string => {
  ...
  if (["active", "activecontext", "recent", "recentcontext"].includes(key)) return "active_context"
