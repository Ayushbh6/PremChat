...
- Main Socrates reads identity, user profile, generated tool guidance, installed skills, project resources, and `.socrates` documents through the shared `read`/`search` resource protocol. Identity and user profile remain read-only: Socrates can propose their curation only through `memory_note`. Skill files are never written through generic `edit`; the always-visible `capability_manager` owns approval-gated skill creation/update/import/removal through the Skill Writer and MCP configuration/removal through the shared MCP service.
