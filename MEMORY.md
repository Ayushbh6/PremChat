...
- V2 does not run the Classic conversation-title rewriter and does not add a capsule-writing LLM. Goal titles and rich, materiality-gated capsule snapshots are derived deterministically from authoritative V2 state; capsule versions carry the objective, latest request/outcome, constraints, open loops, next actions, and evidence anchors needed to resume safely. Goal resolution uses the selected main provider/model through `SocratesAgent.resolveGoal`, the shared runtime and prompt core, zero tools, and the strict four-way schema; there is no router worker setting.
