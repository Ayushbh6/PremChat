...
Automatic typed memory-candidate retrieval keeps recall centered on curated authorized sources and starts concurrently with goal-card and capability retrieval. After the same selected Socrates resolves and the backend binds the exact canonical goal/task, deterministic policy filters duplicates and backend-only/standing sections, reranks against the goal and current request, preserves source diversity, and attaches exact candidate contents with human provenance. No model-driven Memory Router or `memory_search` loop exists. Every user request has one durable task scope across automatic Terminal wait/resume continuations. The same main Socrates performs the work, uses `.socrates` naturally, reconciles important state inside its ordinary tool loop, and returns the strict structured answer in that loop's last continuation. The runtime must not inject milestone or pre-final checkpoint messages. Ordinary workspace-artifact restrictions such as "do not edit files" do not suppress useful `.socrates` work unless the user semantically includes Socrates memory, project notes, internal state, `.socrates`, or all changes whatsoever. Backend-owned notes sections remain invalid mutation targets.
