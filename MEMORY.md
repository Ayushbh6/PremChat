...
- Treat the global goal-centric authority update as the next documentation checkpoint. Do not patch the released Goal Router, Memory Router, project navigation, or automatic compactor piecemeal. First inventory every affected caller/contract/store/UI/test, then propose the exact replacement phases and wait for confirmation before runtime implementation.
