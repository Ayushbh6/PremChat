...
Released state remains recoverable only in the verified cutover archive. The new database begins with no old projects, conversations, goals, messages, tools, traces, or Terminal history.

Read only and Selected approval-gate every Terminal `run/start`; Read only approval-gates all structured mutations, while Selected makes only writes inside selected roots automatic. Structured read/search is global in all modes. Full makes ordinary mutations and side effects automatic. Frontier always requires approval, and the catastrophic protected set is always denied. A native platform adapter contains Terminal children; obvious command/path preflight is defense in depth only. If containment cannot be established, automatic Full Terminal fails closed.

Implementation checkpoint (2026-07-30): `packages/contracts/src/filesystemAccess.ts` owns the shared access contract; `apps/server/src/services/store/accessStore.ts` owns the durable global state, canonical roots, revisions, and immutable turn snapshots; `packages/workspace/src/tools/common.ts` owns symlink-safe structured path resolution; `apps/web/src/components/chat/AccessControls.tsx` is the global `/chat` control surface. Existing primary workspaces are imported non-destructively. Released `runtimeConfig.sandboxMode` is compatibility metadata only and must never become a second authority.

Workspace folders remain real local surfaces, but existing `.socrates/` directories are ignored and left untouched. Resource creation requires an exact user-supplied existing path or confirmation of a discovered candidate. Selected roots never create resources. Do not edit a workspace root `.gitignore` automatically.
...
Existing-path verification, resource-location inspection, containment launches, and resource file operations also belong to `packages/workspace`. The runtime never scaffolds a repository-local `.socrates` directory.
...
The converged main Socrates model-visible tool surface is intentionally small:
...
Do not expose separate `glob`, `grep`, `write`, `git`, `todo`, `question`, broad web-search, or sub-agent/task tools. Those may exist as internal implementation helpers, but main Socrates sees the smaller surface above plus only relevant dynamic MCP tools selected through catalogued capability retrieval or exact `socrates://capabilities` fallback. Patch application remains `apply_patch`, not a hidden patch mode inside `edit`.

Do not dump all skills, MCP servers, MCP tool schemas, workspace runtime facts, or current date/time into the system prompt. Stable routing guidance belongs in the base prompt; changing facts belong behind tools. Retrieve compact capability cards through the shared lexical/semantic/hybrid foundation in parallel with goal and memory candidates. The runtime must not grep user prompts for skill/MCP names. Socrates deepens discovery through `read`/`search`; before claiming no suitable installed capability exists, it must search `socrates://capabilities`.
...
`read`, `search`, and `trace_retrieve` are read-only. Through the governed resource protocol, `read`/`search` cover globally readable filesystem paths, central identity/profile/global-resource knowledge, generated tool guidance, eligible installed skills, confirmed resources, and capability cards. Central knowledge, generated tool guidance, and installed skills are never writable by main Socrates. Knowledge proposals use `memory_note`; skill and MCP lifecycle uses the always-visible `capability_manager`. Policy follows the immutable task access snapshot, while inferred proposals still require semantic acceptance. Never invent packages, URLs, commands, or credentials, and never use Terminal to bypass this lifecycle.

`trace_retrieve` must stay high-level and intent-based. Main Socrates starts with `operation="search"` and uses goal/task-scoped `lexical`, `semantic`, `combined`, or `audit` retrieval. The active goal is the default; exact task narrowing is optional. The main schema must not expose obsolete project/conversation selectors. Lexical queries search all supplied terms and are rejected above 128 characters; semantic/combined queries are rejected above 1,000 characters. There is no silent truncation. Audit-only filters are for raw tool/shell/file/patch/error evidence. The Global Memory Agent receives only exact evidence references attached to memory notes and has no broad goal-retrieval phase.

Normal trace search output must stay slim: `resultNumber`, raw matched `content`, human goal/task/turn labels, `matchedRole`, visible `status`, and `occurredAt`. Knowledge search output similarly exposes numbered content, stable knowledge label, valid section/type, and global/resource scope.
...
Trace retrieval is limited to visible non-deleted goals and tasks. Hard-deleted goals/tasks must not be searchable or inspectable; deletion removes their derived LanceDB parents while canonical deletion/version evidence remains governed by the product retention contract.

Search and inspect results must include enough human goal/task identity to answer correctly. Opaque ids stay backend-resolved unless a typed transport command requires one.

For ordinal recall, the model uses a queryless lexical search with a structured canonical task/turn ordinal, preferably narrowed by the human goal label. The backend must not silently infer an ordinal from natural-language query text. Out-of-range lookup returns no result and must not silently fall back.
...
`socrates://resources` resolves confirmed central resource records and bindings. Search/list output stays to labels, availability, and bounded location metadata until Socrates reads an exact authorized path. User attachments live under `~/.Socrates/artifacts/attachments`, are owned by canonical artifact/message records, and remain distinct from resource roots.

The `edit` tool is the primary model-visible single-file mutation tool. Existing files use one or more exact disjoint replacements in `edits[]`; every match is evaluated against the same freshly read original content, overlaps are rejected, and the file is written atomically once. Whole-file `content` on an existing ordinary file requires explicit `overwrite: true`. The separate `apply_patch` tool covers multi-hunk or multi-file patches. Both show a diff or equivalent preview and follow Read only/Selected/Full autonomy. Model-facing inputs never include `dryRun` or content hashes; preview is runtime-owned. Existing-file edits, patches, deletes, and renames require a prior active-task read, stale hashes are rejected, and another mutation to the same file after success requires a re-read. Central knowledge, generated tool guidance, and skill files remain protected behind their typed owners.

When the user asks Socrates to write code or build something, Socrates creates or edits real files in the relevant confirmed resource through `edit`/`apply_patch`. Temporary plans, probes, diagnostics, and task scratch belong under `~/.Socrates/work/<task-id>`, never in a repository-local `.socrates`. Production code, deliverables, migrations, and permanent tests remain in their proper resource locations. Choose the nearest relevant destination when obvious and ask only when destination, language, or intent is genuinely ambiguous.

Before installing packages or running generated code, Socrates should inspect the confirmed resource when runtime facts matter. Evidence-backed stack and workflow facts belong in versioned resource repository knowledge, not generated workspace files. If a local environment or package-manager workflow is detected, Socrates should use it instead of creating a second environment or running raw global installers. If no environment is detected and dependencies are needed, it should ask before setup unless the user explicitly requested it.
...
Repo-local `.socrates` is not a runtime surface. Existing folders are untouched and ignored. Socrates-owned scratch lives under `~/.Socrates/work/<task-id>`; durable knowledge lives in typed SQLite entries. Ordinary "do not edit files", "make no workspace changes", or "review only" protects user resource files but does not by itself suppress narrowly scoped backend-owned memory curation unless the user also opts out of memory/internal changes.
...
`socrates://knowledge/resources/<resource-id>/repo-facts` exposes versioned central repository facts for a confirmed resource. Root maintainer documentation in this repository lives in `context-files/*.md` and is ordinary source-controlled product doctrine, not runtime user knowledge.

Memory candidate retrieval stays centralized, automatic, read-only, and knowledge-type aware. Its first pass runs concurrently with goal/capability retrieval and includes the current capsule when available. After the same-Socrates goal decision, a changed bound goal or empty eligible first pass may trigger exactly one targeted query through the same service; deterministic selection filters exact candidates by access, bound resources, goal/task, provenance, relevance, and duplication. There is no model-driven Memory Router. The same main Socrates may emit evidence-backed `memory_note` leads inside its normal loop; asynchronous Global Memory work owns curation.

The structured main-Socrates reconciliation/finalization contract is mandatory, but it belongs inside one normal foreground loop. The last continuation from that loop returns the final answer, goal state, and goal note. Do not retain a draft call, detached reconciliation/checkpoint call, separate final-formatting call, final router, or semantic classifier shortcut. The July 2026 `skip_candidate` evaluation remains historical evidence; its fixtures/reports are not runtime authority for extra provider phases.
...
Always-apply rules are typed versioned knowledge capped at 10 active global rules and 10 active rules per confirmed resource. Global hard rules precede resource rules; current explicit user instructions take precedence where policy permits. Explicit user rules activate after validation. Inferred rules remain pending until accepted. Memory Agent merges or supersedes overlap before activation and flags conflicts. The backend attaches only eligible global plus currently bound-resource rules in one stable-prelude snapshot; selected paths never make rules eligible. Current goal/memory/capability context and tool results stay in the dynamic tail. Memory/action ledger messages are forbidden rather than moved elsewhere.
...
Canonical durable knowledge uses shared contracts for global identity/profile/rules and resource memory/rules/repository facts. Stable knowledge ids, versions, provenance, evidence, pending/active/superseded/deleted state, and direct-user mutations live in SQLite. `~/.Socrates/work/<task-id>` is temporary scratch space only. Existing repo-local `.socrates` directories are ignored. `memory_note` submits typed curation leads to the Global Memory Agent. Stable model context order is base prompt, compact identity core, global rules, eligible bound-resource rules, then generated tool guidance; dynamic goal, memory, capability, runtime, attachment, and tool context follows. Per-batch action/memory ledgers and synthetic steering messages are forbidden.

Use `trace_retrieve` for exact task/tool provenance, `socrates://tool-guidance` for generated tool behavior, `socrates://knowledge/global/*` and `socrates://knowledge/resources/<resource-id>/*` for typed durable knowledge, `socrates://resources` for confirmed resource records, `current_time` for current date/time, and `socrates://user/profile` for durable global profile facts.
...
Primary tool-usage docs are runtime-bundled generated Markdown assets, not inline seed strings. They live in `apps/server/src/memory/defaults/primary/tool_usage/`, are copied into server `dist`, install into `~/.Socrates/tool_usage/`, and are read through `socrates://tool-guidance`. Runtime initialization compares content hashes, overwrites every mismatched installed guide, and removes every Markdown file absent from the generated catalog inventory—including otherwise well-formed stale guides—so shadow tool guidance cannot survive.

`socrates://identity` and `socrates://user/profile` provide governed index/section/full reads of global identity and user profile. They are read-only to main Socrates; generic workspace/resource mutation rejects them. Socrates proposes changes only through `memory_note`; Memory Agent replacements require the exact `sectionId` plus section-local source text, validate the complete prospective document, and atomically persist only a structurally valid result. Whole-document replacement is not a Memory Agent authority path. Curation remains evidence-backed and subject to the applicable confirmation/update policy. Applied identity updates create durable user notifications.

The `bash` tool id is the only model-visible command execution tool, but product/UI/prompt copy calls it Terminal. Its required discriminated operations are `run {command,cwd?,timeoutMs?}`, `start {command,name,cwd?,inputMode?}`, `inspect {name}`, `stop {name}`, and `list {}`. Human Terminal name is the only model-facing selector; process/terminal ids, output cursors, character caps, implicit operations, selector aliases, and the alternate argv grammar remain internal or are removed. Policy decides whether each command is auto-allowed, approval-gated, or denied. Internally Terminal remains PTY-backed and platform-native. Destructive, broad network, install, git mutation, delete, migration, and outside-workspace commands require approval by default.
...
Workspace-mutating work must be serialized per workspace across concurrent conversations. `edit`, `apply_patch`, governed project-memory/doctrine edits, and foreground mutating Terminal commands such as Git branch changes/commits/pushes, package installs, migrations, and file-generating scripts use the shared workspace mutation queue. Read-only commands and background Terminals such as dev servers/watchers must not hold that queue forever.
...
`AgentRuntime` is the single released provider-execution abstraction. `SocratesAgent` owns interactive turn policy while focused internal lifecycle modules own retrieval, deterministic memory selection, tools, silent backend counters, approvals, and recovery. The same-Socrates goal-resolution phase configures the same definition and prompt core; it is not another agent role. After binding, one foreground loop owns normal work, optional in-loop reconciliation, and the structured final response. Do not reintroduce a second runner, direct provider orchestration, detached draft/reconciliation/final calls, model-visible action ledgers, or divergent behavior behind similarly named wrappers.
...
- When a branch-heavy structured result is unreliable as a provider schema, define one flat provider-facing wire schema beside the strict domain result in `packages/contracts`, validate it through the shared runtime, and normalize it immediately. The wire projection may not change the semantic outcomes, live in provider-specific code, or become a second decision authority.
...
- The backend attaches exact task/message/evidence refs and confirmed resource bindings. The sending model does not choose scope through prose.
...
Global memory must stay global. Resource-specific facts and rules belong in resource-scoped typed knowledge versions; they never enter the global profile merely because paths were selected.

Skill writing follows the same rule. The Memory Agent or an explicit user request may decide that an approved skill create/update should happen, chooses path-bound/global scope, and uses human-facing skill names. Repeated subject matter is not a skill; repeated ordered work with triggers, phases, decision gates, corrections, and verification is. Every proposal carries inspected source-turn evidence. Final `SKILL.md` authoring belongs to the Skill Writer Agent through `capability_manager`; generic `edit` never writes a skill. The backend determines create versus update from canonical target existence. Main Socrates uses automatic capability matches and, when needed, searches `socrates://capabilities` then reads the best exact skill before ordered multi-step, verification/review, and closure/handoff work.
...
`context-files/GLOBAL_SOCRATES_NORTH_STAR.md` is the product-intent authority. `context-files/UNIFIED_SOCRATES_LIFECYCLE.md` is the detailed lifecycle and cleanup authority. `context-files/AGENT_REFACTOR_MANIFESTO.md` is the normative authority for the agent/runtime/capability boundary, and `context-files/AGENT_CAPABILITY_WORKFLOW.md` is its mandatory operational workflow. Read them completely before related work. Legacy project/conversation records are migration inputs only; deleted alternate-mode reports and adapters are not architecture authority.

Required boundary:

- Preserve the released installation non-destructively in a verified whole-state archive, then start the active database fresh. Do not import old work history.
- `/welcome` exposes one **Open Socrates** action to `/chat`. `/chat` opens one seamless Socrates without onboarding, project, Flow, or conversation selection. The header exposes Paths, visible Read only/Selected/Full access, and Settings. The collapsible sidebar uses flat Goals and Exact exchanges replacement pages with a back control; it has no nested tree or project/conversation level.
- Released project/Flow records remain only in the archive and are never active bootstrap, retrieval, or transport owners. New `/chat` work must not create, select, subscribe to, import, or persist through a Flow/project/conversation. One global state owns the only current-goal pointer and active root task; task access snapshots own authorization and explicit resource bindings own resource provenance.
- The main canvas displays exactly one current or deliberately selected historical exchange. Historical selection is passive: it does not mutate the goal ledger or reopen work, and sending returns to the canonical live tail before the normal four-way same-Socrates decision.
...
- Full access remains visible, explicit, and revocable. It makes ordinary actions automatic but never bypasses Frontier approval, typed credential/clarification waits, or the catastrophic hard-deny boundary.
...
- The target collapsible sidebar is a two-stage navigator. It opens on a flat goal list with the current goal first and searchable earlier/completed goals after it. Selecting a goal replaces that page with the goal's flat exact-Q&A list; a fixed back control returns to goals. Exact Q&A never nests beneath a goal row, and there is no Projects or Conversations level.
- Paths, access mode, and Settings remain fixed header controls rather than sidebar navigation levels. Settings navigates to the existing full `/settings` page, which links to the existing `/memory` Memory Center; do not create a chat-owned settings drawer or duplicate either management surface.
...
- The global center canvas renders one exchange only. Long user text may collapse visually behind **Show more**, but its canonical content remains exact; a new live exchange replaces the prior one on the canvas rather than appending a chat feed.
...
- `Live Work` and `Live Goal` are the only movable poster notes. Work uses secret-safe typed activity; Goal always uses backend-authoritative foreground goal/task state even during historical inspection. Note coordinates/z-order/open state are one global presentation preference, keyboard operable, viewport-clamped, and folded into accessible narrow-screen panels; they never become semantic or persistence authority. No reset-notes control appears on the canvas.
