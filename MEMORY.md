...
- Active build on the unified lifecycle: canonical global resource scopes and header-visible Read only/Selected/Full access, with symlink-safe structured-tool/retrieval/index enforcement, honest approval-controlled local Terminal behavior, non-destructive workspace migration, and no OS process-sandbox expansion. The direct no-project landing/sidebar and one canonical global current-goal pointer follow on the same authority without parallel access paths.
