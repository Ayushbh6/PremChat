...
import type {
  SocratesArtifact,
  SocratesCreateSpeechJobRequest,
  SocratesSpeechJob,
} from "@socrates/contracts"
import { SocratesError } from "@socrates/shared"
import {
  registerSocratesSpeechRoutes,
  type SocratesSpeechArtifactContent,
  type SocratesSpeechJobUpdate,
  type SocratesSpeechPersistence,
  type SocratesSpeechRouteServices,
} from "../routes/socratesSpeechRoutes"
import {
...
class FakeSocratesSpeechPersistence implements SocratesSpeechPersistence {
  readonly artifacts = new Map<string, SocratesSpeechArtifactContent>()
  readonly jobs = new Map<string, SocratesSpeechJob>()
  readonly updates: Array<{ jobId: string; update: SocratesSpeechJobUpdate }> = []
  ...
  createSpeechArtifact(input: {
    ...
    const artifact: SocratesArtifact = {
      id,
  ...
  createSpeechJob(input: {
    ...
    const job: SocratesSpeechJob = input.request.engine === "local_kokoro"
      ? {
          ...common,
          kind: "synthesis",
          engine: "local_kokoro",
...
type TestRuntime = {
  app: FastifyInstance
  persistence: FakeSocratesSpeechPersistence
  packs: {
...
const createRuntime = async (overrides: Partial<SocratesSpeechRouteServices> = {}): Promise<TestRuntime> => {
  const persistence = new FakeSocratesSpeechPersistence()
...
const uploadAudio = async (
...
): Promise<SocratesArtifact> => {
  const boundary = "socrates-v2-speech-boundary"
  const response = await runtime.app.inject({
    method: "POST",
    url: `/api/socrates/speech/artifacts`,
  ...
  return (response.json() as { data: { artifact: SocratesArtifact } }).data.artifact
}

describe("Socrates speech HTTP routes", () => {
  it("exposes only explicit, verified speech pack lifecycle operations", async () => {
    const runtime = await createRuntime()
    const list = await runtime.app.inject({ method: "GET", url: "/api/socrates/speech/packs" })
    ...
    const installed = await runtime.app.inject({ method: "POST", url: "/api/socrates/speech/packs/whisper-small.en/install" })
    ...
    const removed = await runtime.app.inject({ method: "DELETE", url: "/api/socrates/speech/packs/whisper-small.en" })
    expect(removed.statusCode).toBe(200)
    expect(removed.json()).toMatchObject({ data: { removedPackId: "whisper-small.en", pack: { verified: false } } })

    const unavailable = await runtime.app.inject({ method: "POST", url: "/api/socrates/speech/packs/whisper-tiny.en/install" })
    expect(unavailable.statusCode).toBe(404)
    expect(unavailable.json()).toMatchObject({ ok: false, error: { code: "v2_speech_pack_not_found" } })
  })

  it("stores uploaded recordings through the Socrates artifact adapter", async () => {
    const runtime = await createRuntime()
  ...
  it("synthesizes Kokoro audio into a Socrates output artifact and serves it", async () => {
    const runtime = await createRuntime()
    const response = await runtime.app.inject({
      method: "POST",
      url: `/api/socrates/speech/jobs`,
    ...
    expect(response.statusCode).toBe(200)
    const job = (response.json() as { data: { job: SocratesSpeechJob } }).data.job
    ...
    const audio = await runtime.app.inject({
      method: "GET",
      url: `/api/socrates/speech/artifacts/${outputArtifactId}/content`,
    })
...
describe("Socrates speech engine boundaries", () => {
  ...
  it("reports, reuses, and removes an explicitly installed verified pack", async () => {
    ...
    const entry = SOCRATES_SPEECH_PACK_MANIFEST["whisper-base.en"]
  ...
  it("downloads a pack once, verifies bytes and checksum, and removes corrupt staging data", async () => {
    ...
    const fixtureEntry = {
      ...SOCRATES_SPEECH_PACK_MANIFEST["whisper-base.en"],
      version: "fixture-v1",
