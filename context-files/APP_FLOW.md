...
This document defines product routes, page responsibilities, and the currently implemented Classic/Flow navigation. `FLOW_NORTH_STAR.md` owns the target product intent and `UNIFIED_SOCRATES_LIFECYCLE.md` owns the detailed lifecycle/cleanup boundary. `V2_FLOW_ARCHITECTURE.md` records released implementation and migration constraints. Phase 1 removed the detached post-turn Memory Router and mutable main-agent goal ledger; historical phase reports remain baseline evidence only.
...
Both views converge on the same canonical projects, turns, tasks, goals, tools, evidence, live execution state, primary workspace, workspace `.socrates/`, global `~/.Socrates/`, Socrates tool loop, MCP/skills/provider settings, context compressor, and global Memory Agent. Their routing policies and presentations may differ. Phase 3 gives Classic and Flow one canonical task/message identity plus reference-based projections while retaining namespaced physical runtime adapters. Phase 4 completes Projects to Goals to Queries navigation, persistent focus presentation, and the single runtime-authored live-activity to validated-answer choreography. See `FLOW_CONVERGENCE_PHASE_3.md`, `FLOW_CONVERGENCE_PHASE_4.md`, and `V2_FLOW_ARCHITECTURE.md` for the implemented boundary and `FLOW_NORTH_STAR.md` for the durable product authority.
...
The Memory Router keeps recall centered on curated docs instead of raw conversation search. It runs only after the Goal Router has bound the exact canonical goal/task, may make at most three targeted `memory_search` calls, returns only bounded exact file/section `readTargets`, and is strictly read-only. Every user request has one durable task scope across automatic Terminal wait/resume continuations. The same main Socrates that performs the work receives conditional milestone reconciliation checkpoints during long tasks plus one mandatory pre-final checkpoint; it judges, writes, re-reads, and verifies `.socrates` changes itself before its strict no-tool final result. Ordinary workspace-artifact restrictions such as "do not edit files" do not suppress bounded `.socrates` reconciliation unless the user semantically includes Socrates memory, project notes, internal state, `.socrates`, or all changes whatsoever. Backend-owned notes sections remain invalid mutation targets.
...
genuine finalization
  ...
  -> let the router inspect at most three backend-created evidence references
  -> return at most five upsert/replace/remove/archive/condense plans
...
If the pre-turn router remains invalid or fails after its one bounded repair, recall routing is skipped without blocking the ordinary task. The backend persists one `errors` row and records any already-observed usage with failed status, phase, and error linkage. Socrates must not claim the failed recall succeeded; there is no retry queue.
...
V2 uses the same Socrates agent, tools, providers, workspace `.socrates/`, global `~/.Socrates/`, Goal Router, Memory Router, and global Memory Agent as Classic. The shared Goal Router runs first in both views; only after it binds the goal does the shared Memory Router select goal-scoped recall. It does not use the Classic conversation-title rewriter or a capsule-writing LLM: deterministic goal titles and materiality-gated rich capsule versions provide the Seamless navigation/resume labels. The Goal Router has its own model/thinking setting and runs its strict structured contract through the shared agent runner without calling the title-rewrite service. Its normal path receives five bounded candidates and it may make at most three `goal_search` calls, each returning at most three cards. Canonical task and message identity determines the one authoritative physical source. Detailed turn, tool, approval, Terminal, evidence, context, usage, and event records remain in that source runtime's adapter tables and are projected across views rather than duplicated. Canonical Q&A parents reuse shared LanceDB lexical/semantic/combined retrieval under their physical `runtimeKind` and source scope; queryless/inspect/audit continue to resolve through the authoritative source evidence.
