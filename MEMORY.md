...
- Released Goal Router and pre-turn Memory Router implementations that must be removed from the target critical path after parallel candidate retrieval, same-Socrates goal resolution, and deterministic memory selection are proven.
...
- Target recall runs hybrid goal/memory candidate retrieval concurrently, always supplies the current capsule and latest exact exchange independently of score, then deterministically filters exact memory after goal binding. Main Socrates owns deeper exact inspection and required reconciliation.
...
The implemented V2 model has one persistent visible Flow per project. A bounded Goal Router selects one foreground goal, parks other goals as versioned capsules, and supplies the shared Socrates lifecycle with bounded exact/retrieved history. Raw PDF pages, tool results, Terminal output, files, messages, and other evidence remain immutable and retrievable even when their expensive model-context copies are released. Classic and Flow now project canonical work identities and references; legacy mirror rows remain rollback evidence only and are excluded from active reads.
...
- Phase 1 lifecycle convergence is implemented: production keeps only the read-only pre-turn `MemoryRouterAgent` phase (at most eight exact `readTargets`, at most three searches) after goal/task binding. The same main Socrates owns conditional milestone and mandatory final reconciliation, then returns the strict validated answer plus goal state/note for atomic Classic/Flow persistence. Detached post-turn prompts, schemas, tools, telemetry phases, and mutable main-agent goal-ledger authority are removed.
...
- Phase 1 of `codex/agent-core-rebuild` establishes the shared agent core: declarative `AgentDefinition` records, enforced `RoleManifest` and ordered `ContextProfile` contracts, one injectable `ContextPipeline`, one provider-neutral `AgentRuntime`, definition-bound production main/Skill Writer/Global Memory/compactor/title/confirmation roles, and a generated CI inventory. The released Goal Router and Memory Router remain explicitly inventoried removal debt; do not migrate or extend them before their owning lifecycle cutovers.
- Next build the unified `CapabilityCatalog` and canonical tool/capability standard through the mandatory workflow. Replace old registry/schema authorities rather than adding adapters that become parallel workflows, preserve the shared agent core as the only model-execution path, and keep all mutating verification isolated from normal Socrates state.
- Use isolated replay/evaluation state to compare the released sequential-router pipeline against the target: concurrent hybrid candidate retrieval, same-Socrates four-way goal resolution, deterministic exact memory selection, and one foreground loop. Measure goal fragmentation, goal-selection accuracy, memory recall, byte-exact history retention, p50/p95 time to first useful output, full task completion, and provider/tool reliability.
- Build the replacement through the shared agent/capability architecture, cut all production callers over once, prove the global landing/header/sidebar/access experience, then delete the old router agents, goal-search loop, project-first semantic authority, view-specific context paths, and automatic lossy compaction authority. Preserve released user data and compatibility adapters non-destructively during migration.
