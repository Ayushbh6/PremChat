...
Socrates is being rebuilt as one global, goal-centric assistant that the user opens and immediately talks to. Projects and conversations are released migration surfaces, not the target user mental model. Paths, connected resources, and visible Selected/Full access define authorization; goals organize coherent outcomes; exact Q&A remains permanently retrievable beneath versioned goal capsules.

Before any agent architecture, capability, tool, shared utility, prompt, routing, retrieval, embeddings, vector search, context management, provider, worker-agent, generated tool-documentation, goal/task/capsule lifecycle, access scope, reconciliation, ledger, finalization, or UI-navigation work, read root `AGENTS.md`, `context-files/REPO_RULES.md`, `context-files/FLOW_NORTH_STAR.md`, `context-files/UNIFIED_SOCRATES_LIFECYCLE.md`, `context-files/AGENT_REFACTOR_MANIFESTO.md`, and `context-files/AGENT_CAPABILITY_WORKFLOW.md` completely. The manifesto is the agent-core authority and the workflow is mandatory; phase reports are historical. The target sequence is: persist the exact message; retrieve goal, memory, and capability candidates together; same-Socrates decision of current, retrieved older goal, new, or clarify; bind the goal and exact memory; one normal Socrates working loop containing necessary tool continuations, useful notes/memory, reconciliation, and the final structured `answer + goal state + goal note`; atomic persistence; publication; asynchronous enrichment. There is no target separate Goal Router agent, goal-search tool loop, model-driven Memory Router, mutable main-agent ledger, per-batch action-ledger message, hidden checkpoint, detached reconciliation call, or detached final-formatting call. Model-context efficiency is shared runtime policy: append eligible large-result `R<n>` notices to their existing tool results, and automatically compact the oldest model-visible history at 170k while preserving canonical exact sources and an approximately 70k newest whole-turn suffix.
...
- `.socrates/notes` is Socrates' free-working space for plans, tasks, experiments, temporary scripts, and progress notes; `.socrates/memory` stores important project knowledge and decisions; `.socrates/repo_docs` stores verified facts about how the repository works; `memory_note` sends identity, profile, cross-project-memory, or future-skill candidates to the asynchronous Global Memory Agent. Socrates uses these naturally like a human assistant, not ceremonially. Filenames for plans/tasks are not prescribed, and production code plus permanent tests remain in the repo.
- Global Socrates tool guidance and skills under `~/.Socrates/`.
- Exact old conversation/tool evidence through `trace_retrieve`.
- A governed `read`/`search` resource protocol for workspace files, identity, user profile, generated tool guidance, installed skills, project resources, and `.socrates` documents. Durable project memory, notes, and repo-doc base URIs are read/search only; main Socrates writes one exact section URI after reading it, with full-document validation and atomic persistence. Identity and user profile remain read-only to main Socrates; proposed changes go through `memory_note` to the Memory Agent, whose replacements also require an exact section id.
- A shared no-tool `CompressorAgent` that automatically compacts the oldest model-visible history at the fixed 170k trigger and can compact an oversized active turn's oldest completed tool-exchange prefix. It always keeps the active request, pending operations, and newest suffix raw; preserves approximately 70k of newest safe raw context when possible; retains stable canonical source provenance; and never changes canonical exact sources.
- Global Paths/Access is implemented on `codex/agent-core-rebuild` at `647943a`: one durable `Read only`/`Selected`/`Full access` state, canonical and revocable selected roots, one working root, immutable per-turn authorization snapshots, symlink-safe structured filesystem resolution, shared Classic/Flow header controls, and non-destructive import of existing primary workspaces. Full access expands application filesystem scope but never waives approval, destructive-action, credential, publishing, or external-side-effect safeguards. Terminal remains an ordinary local host process whose requested launch directory and independent risk policy are checked; no OS process sandbox was added. Released `sandboxMode` values remain compatibility metadata, not filesystem authority.
...
- Phase 3 final isolated verification on 2026-07-27 passed the full workspace suite (CLI 9, contracts 43, web 26, MCP 14, providers 94 with one deliberate skip, workspace 105, core 126, server 246), full typecheck/build, the 8-agent/69-capability/28-static-tool/24-command architecture check, runtime/native-speech/LanceDB smoke, and disposable OpenRouter DeepSeek V4 Pro current/older/new/clarify plus byte-exact context acceptance. Normal runtime state was not used.
- The 2026-07-28 goal-aware memory follow-up passed the architecture drift check, full workspace typecheck/build, and the complete test matrix: CLI 9, contracts 43, web 26, MCP 14, providers 94 with one deliberate skip, workspace 105, core 128, and server 250. Focused Classic/Flow tests prove current-capsule first-pass queries, one older-goal refinement, no redundant current-goal refinement, deterministic merged reranking, and honest refinement failure. No provider call, runtime archive, or normal Socrates state was used.
...
- Foreground recall runs hybrid goal/memory candidate retrieval concurrently. Its memory query includes the current capsule when available; after goal binding, a changed goal or empty eligible result may trigger one targeted query through the same retrieval service before deterministic reranking. The current capsule and latest exact exchange remain independently guaranteed, and main Socrates owns deeper exact inspection and required reconciliation.
...
- Explicit user memory opt-outs are authoritative across foreground Socrates, automatic indexes, deterministic selection, and asynchronous Global Memory work. Genuine "do not remember/save/store this" intent is interpreted from full semantic meaning rather than literal phrase matching; quoted examples and feature discussions do not trigger it. Clearly scoped opt-outs cover only that content, while broad or ambiguous scope covers the whole source message. Ordinary "do not edit files", "make no workspace changes", or "review only" instructions restrict user workspace artifacts but do not by themselves suppress Socrates internal memory unless the user includes memory, project notes, internal state, `.socrates`, or all changes whatsoever. Opted-out content must not enter goal/memory indexes, durable curation, skill proposals, or memory writes.
...
- Deterministic memory selection never writes. Socrates owns exact project/repo doc mutations, and final answers are blocked until every required `.socrates` reconciliation target has been mutated and re-read. Asynchronous Global Memory work owns approved durable profile/identity curation. The backend deduplicates paraphrased same-turn memory notes before insert.
...
Accepted target model-driven roles remain thin instances of one shared architecture:

- Foreground Socrates is the one global user-facing agent. It works across authorized paths and connections, owns same-turn semantic goal resolution and task execution, and uses scoped project/repo doctrine tools where those surfaces exist. It does not write identity, user profile, or skills directly.
- Asynchronous Global Memory work is the learning curator. It keeps `~/.Socrates/user_profile.md`, `~/.Socrates/identity.md`, retrieval links, and skill freshness up to date from exact completed-turn evidence without delaying foreground work.
...
Goal/memory candidate retrieval, deterministic memory selection, access enforcement, ledger changes, transactions, and indexing are catalogued services rather than extra agent roles.

## Global Socrates North Star And Released V2 Implementation

`context-files/FLOW_NORTH_STAR.md` is the product-intent authority: one global seamless Socrates opens directly from the landing page; Paths, Selected/Full access, and Settings form the minimal header; the collapsible sidebar groups exact Q&A by goals and has no required project hierarchy. Every message creates a task, while only a genuinely independent outcome creates a goal. The current capsule/latest exact exchange are guaranteed context, older goal capsules come from hybrid retrieval, and the same Socrates makes the four-way goal decision. `context-files/UNIFIED_SOCRATES_LIFECYCLE.md` contains the complete technical contract.

Everything below in this section describes released Classic/V2 implementation and migration reality. It must remain safe and recoverable, but it does not override the new target.

The released implementation boundary that must remain safe and compatible during convergence is:

- V1 Classic keeps its established transport, conversation UX, and conversation-scoped STT behavior while participating in the shared canonical goal/task lifecycle. Do not retrofit Flow-only presentation, context-disposition persistence, V2 speech jobs/read-aloud, or V2 transport semantics into Classic conversation/turn/message rows. Classic goal routing is an adapter over the same canonical work state, not a second agent or a copied Flow transcript.
...
- V2 inherits the same Socrates agent behavior, provider/model catalog, role-scoped capability set, Ollama, approvals, Terminal primitives, MCP capability registry, ZIP skill import, attachments, artifacts, workspace serialization, deterministic exact-memory selection, and global Memory Agent. It reads and writes the same workspace `.socrates/` and global `~/.Socrates/` memory/capability surfaces through shared stores.
- The view boundary is below the shared turn lifecycle: V2 retains namespaced transport, presentation, runtime events, and source-owned audit rows, while Classic and Flow use the same resolved context, canonical goal/task state, main tool executors, exact whole-item history selection, reconciliation, and validated finalization authority. Prepared context contains no Classic/Flow label or view-specific history policy.
...
- V2 does not run the Classic conversation-title rewriter and does not add a capsule-writing LLM. Goal titles and rich, materiality-gated capsule snapshots are derived deterministically from authoritative V2 state; capsule versions carry the objective, latest request/outcome, constraints, open loops, next actions, and evidence anchors needed to resume safely. Goal resolution uses the selected main provider/model through `SocratesAgent.resolveGoal`, the shared runtime and prompt core, and zero tools. One flat shared-contract provider envelope is validated and immediately normalized into the strict four-way domain result; there is no router worker setting or second semantic authority.
- The explicit V1/V2 bridge stores projection references only: each V2 focus owns at most one preferred Classic home, and **Open in Classic** / **Continue in Flow View** render the same canonical messages and task state without replacement Q&A, tool, evidence, usage, or event copies.
- Goal lifecycle is `foreground`, `parked`, `blocked`, `completed`, or `archived`. The UI exposes switch, pause, finish, reopen, archive, pin, and unpin; Socrates returns the bound goal's final state/note only in its validated structured result, which commits atomically with the substantive answer. General Conversation cannot be completed/archived, and unpinned parked work auto-archives after seven inactive days.

The implemented V2 model has one persistent visible Flow per project. The same configured Socrates selects the current, a numbered older, a new, or an ambiguous goal after concurrent retrieval; deterministic code applies one foreground pointer and parks other goals as versioned capsules. Raw PDF pages, tool results, Terminal output, files, messages, and other evidence remain immutable and retrievable even when their expensive model-context copies are released. Classic and Flow now project canonical work identities and references; legacy mirror rows remain rollback evidence only and are excluded from active reads.

The shared foreground loop has one turn-local context-efficiency action: `release`. Each successful individual tool result over 3,000 estimated tokens receives the next friendly handle (`R1`, `R2`, ...), with one compact reminder appended to that existing tool result rather than a separate hidden message. When Socrates has extracted what it needs and is already making another normal tool call, it may piggyback `context_disposition({ release: ["R1"] })` in the same response. Release never adds a model round trip, never blocks functional calls when omitted, is excluded from the functional-tool budget, affects only the current model-visible projection, and leaves exact audit/retrieval evidence immutable.

Speech shares one replaceable transcription foundation without sharing orchestration. Classic uses the existing composer plus a push-to-talk mic and the temporary conversation endpoint `/api/projects/:projectId/conversations/:conversationId/speech/transcribe`; its current UI defaults to local Whisper `small.en`, appends the result to the draft, never auto-sends, and records no V2 artifact/job/Flow state. A finalized V2 transcript enters the same unified pre-turn lifecycle as typed text and additionally supports local Whisper `base.en`/`small.en` or OpenRouter restricted to `nvidia/parakeet-tdt-0.6b-v3`, `microsoft/mai-transcribe-1.5`, and `mistralai/voxtral-mini-transcribe`. Local STT uses exact-pinned `@fugood/whisper.node@1.0.22` by default, with an explicit `whisper-cli` override/recovery path. Granite Speech is not included. A local transcription failure never uploads audio without explicit user selection of the hosted route.
...
- `edit` updates ordinary workspace files and authorized exact `.socrates` project-document section URIs through the governed resource adapter. Durable base URIs are read/search only; the old whole-document project/repo append and replace operations are removed. Identity, user profile, generated tool guidance, runtime-owned sections, and skill files are denied.
...
- Unified lifecycle and foreground-loop convergence are implemented: Classic and Flow start goal-card, current-capsule-aware memory, and capability retrieval together; the same selected Socrates makes only the four-way no-tool goal decision; deterministic code binds the goal and exact authorized memory; and one view-neutral prepared context feeds one normal foreground loop. That loop owns real tool continuations, conditional in-loop reconciliation, and its strict terminal `answer + goal state + goal note`. The same streamed provider request natively enforces that terminal schema and counts it as model input. Detached router, draft, reconciliation, repair/final-formatting calls plus per-batch action/memory steering are removed and protected by architecture checks.
...
- The Socrates prompt envelope stays cache-friendly: stable system instructions first, then the stable always-apply prelude, then the unchanged exact history prefix, with current goal/memory/capability context, the latest user message, and tool results in the dynamic tail. No changing date/time, workspace scan block, skill/MCP counts, volatile ids, per-batch ledger, or hidden checkpoint belongs in the stable prefix or model conversation.
...
- Project docs, repo docs, identity, and user profile updates get backend-owned frontmatter stamps (`updated_at`, `updated_by`, `last_edited_section`) only after the full prospective document validates and is atomically persisted. Model-written prose should not invent "today" when these system stamps are enough.
- Governed `edit` may update exact project-memory/notes and repo-doctrine section URIs while protecting base documents and backend-owned sections. Generic `edit`/`apply_patch` writes to the durable raw files, `.socrates/skills/**`, global identity/profile/tool guidance, and runtime-owned state remain rejected; approved skill mutation uses `capability_manager` and the Skill Writer/importer path. Free `.socrates/notes/**` and `.socrates/work/**` files remain ordinary working space.
- Socrates loads applicable repo doctrine and project state when relevant, then reconciles important `.socrates` state inside the same normal working loop before its structured answer. A durable-state change means a material goal/scope change, a decision future work depends on, a verified build/test milestone, a blocker, an incomplete handoff, or restart state. Ordinary repeated reads, searches, Terminal calls, and file edits do not each trigger a document preflight, memory review, hidden checkpoint, or extra model call.
...
- Global Paths/Access follows the same honest boundary: selected roots are enforced for structured filesystem tools, retrieval, and indexing; Terminal remains approval-controlled local execution under the host user. This phase must not add or claim a container, VM, native helper, or OS-specific process sandbox. Read-only denies mutation and Terminal; Full Access expands application-level filesystem scope but never waives destructive, credential, publishing, or external-side-effect safeguards.
...
- Interactive Terminal is now a verified end-to-end contract, not only a PTY primitive. User-interactive requests use a named `start` session with portable Node.js/Python stdin instead of shell-specific `read -p`; when a prompt is visible, the completed/failed wait is registered deterministically so provider noncompliance cannot strand the turn. A durable-state reconciliation occurs only when the interactive result materially changes the goal state, decision record, milestone, blocker, or final handoff. Quick foreground commands retain output captured during initial exit rather than double-consuming the model cursor. Current runtime Terminal capabilities override stale project memory that claims interaction is unavailable. The UI renders a resumed suspended phase as `Continued after Terminal completed`. Real DeepSeek V4 Pro, thinking-off browser verification passed two separate inputs (`green`, then `green parrot`), dependent Q2 construction, clean exit, automatic resume, and final answer.
...
- Memory-agent tool guidance is generated under `apps/server/src/memory/defaults/primary/tool_usage/memory_agent/`, installed under `~/.Socrates/tool_usage/memory_agent/`, and read through the shared governed resource path. The entire installed tool-guide tree exactly mirrors generated catalog content by hash and inventory; stale non-catalog Markdown is pruned.
...
- Worker model settings live behind `/api/worker-model-settings` and are surfaced on the Settings page for five independent roles: Skill Writer, Socrates Context Compactor, Memory Context Compactor, Title Generator, and Frontier. The selected main Socrates model also owns semantic goal resolution; no Goal Router or Memory Router selector exists. Settings persist provider, auth mode, model, and thinking choice. Frontier defaults to OpenRouter `x-ai/grok-4.5` with low reasoning because OpenRouter marks Grok 4.5 reasoning as mandatory and rejects disabled/none requests; its valid options are Low, Medium, and High. Saved thinking selections that a model no longer supports resolve to that model's supported default instead of sending an invalid provider request. The main Socrates model is the primary worker and may request `handover_to_frontier({ focus? })` only after real substantive effort reaches a concrete unresolved capability/reliability blocker. Every request requires explicit typed user approval even in approve-all/full-access mode and uses polished `Call Frontier model` UI copy. Approval transfers the exact current model-visible working context to Frontier for the remainder of that turn—including prior automatic compaction and turn-local releases—while canonical exact evidence remains retrievable; it discards provisional driver answer text and persists the normal tool call plus `agent.model.handover`. Rejection persists the rejected approval/tool call, blocks another request for the turn, and instructs Socrates to continue and complete the work itself. The next user-authored turn starts on the selected main model again. There is no consult mode, reason field, return handoff, or multi-agent dialogue. Other defaults preserve the current working models, while credential-aware resolution may prefer ChatGPT Codex role defaults when connected. Explicit Ollama selections resolve through the same refreshed model catalog and use Ollama's Off/On thinking toggle.
...
- Skill writing now routes through the production Skill Writer Agent path with a dedicated prompt, shared `AgentRuntime`, a role-scoped `CapabilitySet`, and the catalogued `skill_write` validation/write capability.
- Always apply this creation invariant: every new model-driven agent, router, or worker must use a designated prompt module, `AgentDefinition`, shared `AgentRuntime`, an explicit catalog-backed `RoleManifest`/executor mapping, strict Zod contracts, bounded structured-output repair/fallback, dedicated model/thinking settings when independently configurable, typed telemetry/persistence, and focused tests. One-off provider calls, inline production prompts, ad hoc tool arrays, and borrowing an unrelated worker role are prohibited shortcuts; existing violations must be corrected and never copied.
...
- Direct DeepSeek function-tool parameters must always be top-level JSON Schema objects. `packages/core/src/capabilities/providerProjection.ts` derives each `providerInputSchema` once from the canonical Zod input schema and preserves exact operation variants while adding only the required top-level object marker for unions. Every provider consumes that catalog projection unchanged; providers have no tool-name schema branches or normalization copy. `packages/core` validates each call against the same canonical schema before execution.

## Context Compression

- Automatic compression is triggered at 170k estimated model-visible input tokens for both normal Socrates chat calls and backend Global Memory Agent calls. It is backend lifecycle behavior and never pauses for user approval.
- Compaction targets a rebuilt request around 100k, accepts only results at or below the 120k post-compaction safety ceiling, and never dispatches the main model above 170k if safe compaction fails. The older head becomes one provenance-linked hidden summary while canonical exact rows remain unchanged and exactly retrievable.
- The newest approximately 70k tokens of safe raw context remain exact after reserving the stable prefix, original active request, pending operations, and newest active tool-exchange suffix. Completed Q/A use whole-turn boundaries; an oversized active turn may be split only between fully completed tool-exchange batches, never inside a call/result group. Small fixtures use proportional thresholds.
- Current-turn large tool outputs receive qualifying-only turn-local `R<n>` handles. Socrates may release irrelevant handles with its next otherwise-needed tool call; the release only prunes live model context and never the exact persisted evidence. Independently, automatic compaction may summarize only an oversized active turn's completed prefix while its request, pending work, and newest suffix remain raw.
- Older head context is compacted through `CompressorAgent` and `AgentRuntime`, never a direct provider call or streamed prompted JSON.
...
- Model-visible anchors use stable canonical `Turn <number>:` labels and optional task ordinals mapped internally to exact `turnId`/message/tool-batch references. Prior important anchors are carried and deduplicated deterministically; the model does not regenerate ordinals from a filtered projection. Bounded high-value source artifacts are carried forward deterministically even when a model omits them: exact `.socrates/attachments/...` paths enter `relevantFiles`, exact shell commands enter `toolState`, and explicit unresolved/do-not-complete user instructions enter `blocked`.
- An active snapshot is applied before token counting and provider dispatch: its hidden summary is prepended once, only exact persisted entries represented by its compaction boundary are omitted from the model request, and the authoritative raw rows remain in SQLite for trace/audit retrieval. Partial active-turn snapshots never omit the retained suffix merely because it shares the same `turnId`.
- One final shared result guard covers every model tool. Dynamic MCP output is persisted exactly before projection, defaults to approximately 4,000 estimated tokens, never exceeds 6,000, and returns exact continuation/retrieval metadata through existing read/trace surfaces.
- The server chooses Classic/Flow automatic compression through the independent Socrates Context Compactor setting; Global Memory Agent and Skill Writer compression use the independent Memory Context Compactor setting. Migration `0028_split_context_compactors.sql` copies the prior shared `context_compactor` choice into both roles before removing the legacy row. Both built-in defaults are OpenRouter `deepseek/deepseek-v4-flash` with thinking off. When ChatGPT Codex is connected and a saved compactor setting is the built-in default or unavailable, its effective model is ChatGPT Codex `gpt-5.4-mini` with low reasoning. Hard-coded OpenRouter fallbacks must not run when OpenRouter is unavailable.
- Classic conversation titles are generated by the no-tool `TitleGeneratorAgent` through `AgentRuntime`, the prompt in `packages/core/src/prompts/titleGeneratorPrompt.ts`, an explicit empty registry/executor mapping, and `conversationTitleAgentOutputSchema`. The server adapter only loads bounded image content, invokes the agent, sanitizes the validated title, and persists usage/title effects.
...
- Main model assembly keeps a byte-stable prefix: base prompt, compact identity core, global always-apply rules, project always-apply rules, then the generated surface map and unchanged exact history prefix. User/project metadata, routed memory, runtime facts, and tool results follow as dynamic context. Per-batch action/memory ledgers and synthetic steering messages are forbidden.
...
## Current Tool Reliability Verification

- The reported edit/Terminal latency issue was corrected and verified in the real browser on 2026-07-22 without a model-specific branch. Shared contracts, provider input shaping, and the core tool runners normalize legacy/mixed Bash model input before the strict runtime schema, preventing the former `command` plus `argv` rejection and its long retry/backoff. The same lifecycle is used by every selected model.
- The shared Classic/Flow transcript now keeps active, approval, failure, cancellation, Terminal output, and edit-diff activity visibly expanded. It names the work in progress and reports compact edit line deltas, so a rejected or failed tool call remains explicit instead of appearing as a silent long wait.
- Real isolated `Test-Workspace` E2E used the exact AI_DPA landing-page prompt with GPT-5.6 Terra / ChatGPT Codex at Low in Classic and Flow, plus MiMo-V2.5-Pro. Terra Classic performed source edits, approved production build, Terminal launch, and browser inspection with no mixed-Bash retry. The Flow run performed project reads, approved build, Terminal launch, dynamic browser navigation/snapshot/console/screenshot, and restored its backend-authoritative focus note after reload. MiMo completed the same source-edit/build/Terminal/browser route after its catalog capability was corrected to text-only; its current OpenRouter endpoint rejects image parts, so screenshot pixels are no longer sent to it.
- The Flow bridge validates a goal's preferred Classic home against live bridge/conversation/session rows, self-repairs stale homes, and updates the preferred home when Classic routing creates a current bridge. This prevents a stale Classic pointer from blocking a Flow conversation.

## Release State

- The `v0.1.20` release remains deliberately **halted**. The reported workspace-tool reliability/latency issue has now been corrected and product-path-verified, but no user authorization has resumed the release. The release-preparation commit `be29e36` remains on `main`, the local and remote `v0.1.20` tags were deleted, GitHub Actions run `29873514078` was cancelled before the publish job, no `v0.1.20` GitHub Release exists, and npm `latest` remains `@socrates-ai/cli@0.1.19`. Do not recreate/tag/publish `v0.1.20` until the user explicitly resumes the release.
- Current GitHub runtime release is `v0.1.19`, published non-draft and non-prerelease with the three runtime archives plus `SHA256SUMS`. The public npm registry now serves `@socrates-ai/cli@0.1.19` as `latest`; the launcher continues to resolve the newest GitHub runtime by default.
...
- Context compaction now uses a single-pass sizing policy: 80k or less is `excellent`, 100k or less is `preferred`, 100-120k is `acceptable`, and results above 120k are rejected. The 170k trigger is also the fixed model-dispatch ceiling. Recent completed history is retained by whole-turn boundary toward an approximately 70k exact suffix after the fixed prefix and active turn are reserved; this does not add a second compressor call or mutate the active turn.

## Next Major Work

- Phases 1-3 of `codex/agent-core-rebuild` establish the shared `AgentDefinition`/`AgentRuntime` core, one generated `CapabilityCatalog`, and the unified Classic/Flow pre-turn lifecycle. The released router agents, their tool loops/settings/prompts, and view-specific prepared-context authority are now removed; do not recreate them under compatibility aliases.
- Completed foundation on `codex/agent-core-rebuild`: the separate documentation checkpoint `d25fa9b` and implementation checkpoint `647943a` establish canonical global resource scopes and header-visible `Read only`/`Selected`/`Full access`, with symlink-safe structured-tool enforcement, immutable per-turn authorization, honest approval-controlled local Terminal behavior, non-destructive workspace migration, and no OS process-sandbox expansion. Do not rebuild this through another access store, project permission layer, or runtime `sandboxMode` authority.
- Next product phase: replace the project-first landing, route, and sidebar entry experience with the direct global shell from `FLOW_NORTH_STAR.md`: `Open` enters one seamless Socrates, the minimal header reuses the implemented `Paths | Access | Settings` controls, exact Q&A is grouped by goals, and one canonical global current-goal pointer replaces view-owned foreground pointers. Classic/project/Flow routes remain migration adapters during the cutover, not parallel semantic authorities.
- After that UI migration, run natural multi-turn browser/provider acceptance and restart/reliability testing across OpenRouter DeepSeek V4 Pro and ChatGPT Codex Terra. Cover goal continuity and switching, exact memory and `.socrates` use, Read only/Selected/Full behavior, approvals and Terminal, MCP/skill discovery, oversized first-turn and repeated compaction recovery, interruption/restart, and exact trace retrieval. Existing deterministic CI and the isolated provider acceptance below are a foundation, not proof of the final global product journey.
- Keep automatic 170k oldest-head compaction shared across Classic and Flow, with an approximately 70k exact whole-turn suffix, a roughly 100k rebuilt target, a 120k acceptance ceiling, exact canonical preservation, and no provider dispatch above the trigger when safe compaction fails.
- Extend isolated evaluation of the implemented lifecycle across longer natural conversations and providers. Measure goal fragmentation, four-way accuracy, memory recall, byte-exact history retention, p50/p95 time to first useful output, full task completion, and provider/tool reliability without reintroducing semantic routers.
...
Latest verified for the Global Paths/Access implementation on 2026-07-29:

```text
pnpm typecheck
pnpm check:agent-architecture
  -> 7 agents, 67 capabilities, 19 static tools, 28 typed commands
pnpm test
  -> CLI 9, contracts 42, shared 3, web 26, MCP 14, providers 97 (+1 intentionally skipped), workspace 113, core 132, server 263; 699 passed total
pnpm build
git diff --check
isolated disposable OpenRouter DeepSeek V4 Pro acceptance
  -> two real shared-runtime tool calls plus the validated structured final response passed without using normal Socrates state
```
...
npm view @socrates-ai/cli dist-tags version --json
  -> public npm latest and version are both 0.1.19
```
