...
All compressor calls use `CompressorAgent`, `AgentRuntime`, mode-specific prompt modules, an explicit empty tool registry/executor mapping, structured generation, and strict schema validation before a compaction snapshot can become active. Server fallback selection is credential-aware: the runtime may add the available default model as a fallback when it differs from the primary, but hard-coded OpenRouter fallbacks must not run when OpenRouter is unavailable.
