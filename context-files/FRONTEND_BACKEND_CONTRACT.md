...
`classic_only` detaches the Classic home and preserves canonical Flow history. `everywhere` atomically removes linked canonical Flow exchanges before deleting Classic state and then rebuilds project retrieval. Flow turn deletion removes the complete exchange, including its user/assistant messages and owned tool, approval, Terminal, evidence, usage, and projection-reference rows. Flow goal deletion applies that operation to every turn in the focus, removes its Classic projection, and switches to protected General Conversation. Active work cannot be deleted.
...
When the context grows too large, compression happens before a provider request is sent. Classic and Flow are two views of the same Socrates runtime and share one fixed trigger and dispatch ceiling at 170k estimated model-visible input tokens; model context-window metadata must not derive a separate Flow trigger. The rebuilt request targets about 100k, with at most 80k classified `excellent`, 80-100k `preferred`, 100-120k `acceptable`, and anything above the 120k post-compaction ceiling rejected. Tail selection preserves recent completed Q/A by whole-turn boundary toward an approximately 70k exact suffix after reserving fixed prompt/tool context, the active turn, and the maximum summary allowance; it does not spend another model call solely to improve the size class. Recent visible conversation turns remain normal role-typed messages. Only the oldest completed-turn head may become hidden compacted context with validated source-turn handles and targeted retrieval hints. The active turn is not automatically distilled: successful individual tool outputs above 3,000 estimated tokens receive turn-local `R<n>` handles, and Socrates may release irrelevant handles with its next otherwise-needed tool call. Release changes only live model context; exact tool evidence remains persisted and retrievable. Active snapshots replace represented completed raw turns in the model request without deleting SQLite history. Bounded deterministic carryover writes exact `.socrates/attachments/...` paths to `relevantFiles`, exact shell commands to `toolState`, and explicit unresolved/do-not-complete user instructions to `blocked`. Global Memory Agent runs use the memory-specific structured compaction schema and prompt. All compressor calls run through the no-tool `CompressorAgent` and shared structured runner. Classic/Flow uses the resolved Socrates Context Compactor setting; Global Memory Agent and Skill Writer compression use the Memory Context Compactor setting. Built-in OpenRouter defaults must resolve away from OpenRouter when only ChatGPT Codex or another provider auth source is available.
