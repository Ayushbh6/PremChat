...
- Every turn persists the exact user message, retrieves goal and memory candidates in parallel, performs one same-Socrates goal decision (`current`, retrieved older goal, `new`, or `clarify`), deterministically selects exact memory, runs the shared Socrates loop, commits answer/task/goal/capsule state atomically, then enriches memory asynchronously.
...
- Exact user messages, visible assistant answers, explicit constraints, and selected relevant history must never be clipped, summarized, compacted, or omitted silently. Pagination and goal scoping are allowed only when exact source text remains intact and retrievable. Any lossy context transformation requires the user's explicit approval before the provider call.
- Never use the phrase `bounded context` by itself in architecture, product copy, logs, or handoffs. State the exact mechanism instead: `exact scoped selection`, `exact pagination`, `lossless derived index`, or `lossy user-approved compaction`.
