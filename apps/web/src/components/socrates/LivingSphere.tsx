...
export type SocratesPresence = "offline" | "idle" | "listening" | "routing" | "thinking" | "working" | "awaiting_input" | "complete" | "error";

type LivingSphereProps = Readonly<{
  state: SocratesPresence;
  size?: "compact" | "full" | "mini";
  statusLabel: string;
  showStatus?: boolean;
}>;

const stateClass: Record<SocratesPresence, string> = {
  ...
  return (
    <div className={styles.sphereAssembly} data-size={size}>
      <motion.div
        ...
        animate={reduceMotion || !active ? undefined : { opacity: [0.16, 0.28, 0.16], scale: [0.98, 1.035, 0.98] }}
