...
import { useSocratesSpeechPacks } from "@/lib/socrates/useSocratesSpeechPacks";
import {
  SOCRATES_SPEECH_PACK_CATALOG,
  SOCRATES_SPEECH_PACK_IDS,
  type SocratesSpeechPack,
  type SocratesSpeechPackId,
} from "@/lib/socrates/speechPacksApi";
import styles from "./speechPacks.module.css";

interface SocratesSpeechPackManagerProps {
  className?: string;
  headingId?: string;
}
...
const PackIcon = ({ packId }: { packId: SocratesSpeechPackId }) =>
  packId === "kokoro-en-v0_19"
    ? <Volume2 aria-hidden="true" />
    : <Mic2 aria-hidden="true" />;

export function SocratesSpeechPackManager({ className, headingId = "socrates-offline-voice-title" }: SocratesSpeechPackManagerProps) {
    ...
    >
      <header className={styles.header}>
        ...
        <div className={styles.headerStatus}>
          <span>{installedCount} of {SOCRATES_SPEECH_PACK_IDS.length} ready</span>
          <button
            type="button"
      ...
      <ul className={styles.packList} aria-live="polite">
        {SOCRATES_SPEECH_PACK_IDS.map((packId, index) => {
          const pack = packsById.get(packId) ?? { id: packId, installed: false, verified: false, path: "" };
          const detail = SOCRATES_SPEECH_PACK_CATALOG[packId];
