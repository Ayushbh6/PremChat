...
V2 Flow is a separate experimental Seamless web window/mode in the normal frontend/backend product. It has one persistent Flow per project, backend-managed goals, one foreground goal, parked goal capsules, and text/voice entry through the unified lifecycle. Classic and Flow start typed goal/memory retrieval concurrently, use the selected main Socrates for the no-tool four-way goal decision, apply deterministic exact-memory selection, and feed one view-neutral exact context into the shared loop. Flow additionally stores exact namespaced evidence for audit/retrieval but does not automatically inject prior-turn tool outputs into later model requests. Its backend routes remain unmounted unless `SOCRATES_V2_FLOW_ENABLED=true`; disabling that UI flag does not restore the retired Classic-only semantic path. The ordinary NPM/runtime launcher defaults the flag to `true`, while a direct source-server run must opt in explicitly. V2 uses namespaced routes/contracts/state and never silently reinterprets existing V1 chats.

Both views converge on the same canonical projects, turns, tasks, goals, tools, evidence, live execution state, primary workspace, workspace `.socrates/`, global `~/.Socrates/`, Socrates tool loop, MCP/skills/provider settings, context compressor, global Memory Agent, goal policy, memory-selection policy, and exact-history policy. Their presentations and physical transport adapters may differ. The canonical projection foundation gives Classic and Flow one task/message identity; Phase 3 adds the unified pre-turn lifecycle while retaining namespaced physical runtime adapters. See `FLOW_CONVERGENCE_PHASE_3.md`, `FLOW_CONVERGENCE_PHASE_4.md`, and `V2_FLOW_ARCHITECTURE.md` for the implemented boundary and `FLOW_NORTH_STAR.md` for the durable product authority.
...
current memory-agent architecture:
  ...
  concurrent automatic memory-candidate retrieval plus deterministic exact selection
  same-Socrates milestone and final reconciliation
...
`url_fetch` handles exact HTTP(S) URL reads as bounded text or metadata. It is for specific docs pages, redirects, JSON/CSV/plain text, and comparison against local files. It does not search the web, crawl links, save remote files, or return binary bodies; broad web research belongs to configured search/MCP/provider capabilities.
...
`bash` is the compatibility id for the Terminal tool. It runs platform-native PTY commands from the active project workspace, while UI/product copy says Terminal. `operation: "list"` returns at most 12 compact named rows and must be used when the current Terminal inventory is unclear. `run` is a unified PTY path: a quick command returns normally, while any raw shell run still active after `SOCRATES_TERMINAL_AUTO_DETACH_MS` (default 15 seconds) becomes the same conversation-scoped Terminal without being killed or restarted. `start` remains the immediate-background option; interactive programs declare `inputMode: "user"` so input state does not depend on naive prompt words. Rolling PTY analysis only accepts conservative protocol shapes and is bounded to a 2,000-character frame. Each PTY is owned by an independent host process behind a restartable coordinator, so coordinator loss no longer destroys running Terminals. The chat UI exposes one tabbed bottom dock only and opens it automatically for required input. `status`, `output`, and `stop` use human names only; opaque ids and cursors remain internal. Model-visible Terminal output is capped at 16,000 characters, list text at 12,000 characters, and durable full output remains in the UI/audit store.

Conversation terminals are scoped by `projectId + conversationId + workspacePath`. Creation is a durable `starting` phase that becomes `running` only after supervisor ownership and process metadata are recorded. Healthy committed Terminals are task-aware state, not fixed-age resources: a Terminal referenced by a running or waiting Socrates task is not terminated after two hours or merely because the main server restarts. Coordinated server close rejects new Terminal work, aborts and drains active turns, waits for in-flight starts, physically stops incomplete starts, then stops polling while leaving committed sessions owned by the independent Terminal supervisor. Startup reconnects `starting`, `running`, and `awaiting_input` rows, drains persisted output, and resumes monitoring; a live incomplete start is recovered and a missing one is finalized deterministically. Idle supervisors self-expire. Cleanup remains explicit stop, conversation deletion, workspace invalidation, confirmed process loss, deliberate completed-output retention, or an explicit supervisor shutdown; an unsuccessful physical stop is recorded as `detached` instead of `stopped`. `wait` is a separate event-driven model tool. After independent work is exhausted, Socrates registers named Terminal dependencies with `completed`, `failed`, and/or `input_required`; the backend durably suspends the task and ends the model invocation without a final answer. The deterministic coordinator wakes a continuation of that same task only on a requested event, with bounded new output, exit/status evidence, and an authoritative bounded overview of the task's prior tools, commands, Terminal final states, and waits so the fresh invocation does not recreate completed work. Wake claim and continuation claim are atomic. If the server stops after a continuation claim but before that continuation finishes, startup cancels the stale turn and requeues the same durable task for one new continuation attempt. There is no model-facing polling interval. `wait.reason` is required, no more than 7 words and 64 characters.

Each Socrates home owns a restartable Terminal coordinator socket, while every Terminal PTY is owned by its own detached host process at a deterministic scoped socket. The coordinator health handshake reports its instance, process, start time, and known host count. Ordinary operations never replace a reachable coordinator merely because one request timed out; if the coordinator is actually killed, the client restarts it and the new coordinator reconnects to surviving Terminal hosts. Background polling still requires three consecutive failures before declaring a session uncontrollable. Startup reconciliation records whether each active Terminal reconnected or its independent host was genuinely missing. Only a confirmed missing host becomes a deterministic `failed` wake.
...
Terminal lifecycle state is monotonic across startup, PTY output, input, polling, WebSocket delivery, and reload hydration. Each real lifecycle transition increments `stateVersion`; creation commits `starting` before host launch and `running` only after supervisor/process ownership is durable. Input commits `running` before its bytes reach the PTY, prompt detection is generation-bound, unchanged polls emit no duplicate status event, and the frontend rejects snapshots older than the version it already holds. This prevents an interrupted start from looking committed and prevents a delayed `running` event from visually overwriting a newer second `awaiting_input` prompt.
...
Automatic typed memory-candidate retrieval keeps recall centered on curated authorized sources and starts concurrently with goal-card retrieval. After the same selected Socrates resolves and the backend binds the exact canonical goal/task, deterministic policy filters duplicates and backend-only/standing sections, reranks against the goal and current request, preserves source diversity, and attaches at most eight exact candidate contents with human provenance. No model-driven Memory Router or `memory_search` loop exists. Every user request has one durable task scope across automatic Terminal wait/resume continuations. The same main Socrates that performs the work receives conditional milestone reconciliation checkpoints during long tasks plus one mandatory pre-final checkpoint; it judges, writes, re-reads, and verifies `.socrates` changes itself before its strict no-tool final result. Ordinary workspace-artifact restrictions such as "do not edit files" do not suppress required `.socrates` reconciliation unless the user semantically includes Socrates memory, project notes, internal state, `.socrates`, or all changes whatsoever. Backend-owned notes sections remain invalid mutation targets.

Always-apply rules are a tiny curated layer, not a new memory store. Treat them as one centralized always-apply rules list with two lanes: `user_profile.md` has a capped `Global Always-Apply Rules` section, and workspace project memory has a capped `Project Always-Apply Rules` section. Each section is limited to 10 human-readable rules. The backend assembles those two sections plus `core_identity`, `voice_and_presence`, and `relationship_to_user` into one per-project stable-prelude snapshot. Unchanged file stats take the fast path; a changed file triggers content hashing and parsing, but same-content rewrites or changes outside the standing sections retain the cached snapshot. Only a changed standing-section hash rebuilds it, and the cache uses least-recently-used eviction. The byte-stable prelude order is identity core, global rules, project rules, then the compact map generated from `packages/contracts/src/socratesSurfaces.ts`. It is inserted before conversation/user text without five model-visible read calls. Deterministic memory selection excludes standing sections and repeated exact file/section targets; only selected dynamic sources appear afterward.
...
`skills` is a model-visible discovery/inspection tool for enabled builtin, global, and project skills. The preferred model path is `skills({ operation: "list" })` with an optional `scope`, followed by `skills({ operation: "describe", id: "<exact-listed-id>" })` or an exact listed `name`. `list` returns compact rows with id/name/scope/description; `describe` returns the selected `SKILL.md` content; `read` loads one exact relative supporting file referenced by that skill. For ordered multi-step work, verification/review sequences, and closure/handoff requests, main Socrates must perform this discovery before domain tools. The main Socrates agent cannot create, edit, import, or delete skills through this tool. Skill creation/update routes through explicit UI approval plus the Skill Writer Agent, while pre-made ZIP import is a deterministic user-owned backend flow with no authoring model.

Global Memory Center and project dashboard `Skills +` offer `Build with Socrates` and `Import ZIP`. ZIP import stages one standards-compatible skill directory, parses real YAML frontmatter (`name`, `description`, optional `license`, `compatibility`, `metadata`, and experimental `allowed-tools`), previews bounded files/metadata/security warnings, and requires confirmation before installation. Import never executes package content, never grants approval from `allowed-tools`, rejects traversal, symlinks, encryption, multiple roots, oversized packages/files, `.git`, `node_modules`, and reserved provenance spoofing, then installs through a same-root incoming directory plus atomic replacement/rollback. Imported/generated provenance and enable state are stored in `.socrates-skill.json`. Disabled skills stay manageable in UI but disappear from Socrates discovery.

`project_docs` is the constrained read/search/index/edit interface for `<workspace>/.socrates/MEMORY.md` and `<workspace>/.socrates/PROJECT_NOTES.md`. Memory is durable cross-conversation project state; notes are active working state such as todos, active project context, checked files, next commands, and restart points. `runtime_context` and `state_ledger` are backend-owned protected sections: the former refreshes workspace facts lazily, while the latter is rewritten from structured turn data and collapses duplicate legacy ledger blocks to one bounded snapshot. Runtime project docs are structured markdown with stable section ids, so agents should prefer `read_index`, `read_section`, and `patch_section` when the section is known. Tool outputs include system runtime date/time metadata, and successful docs mutations stamp frontmatter with backend-owned `updated_at`, `updated_by`, and `last_edited_section`. Terminal output and live terminal state do not belong in runtime context. After meaningful work, the runtime injects one bounded checkpoint if `project_docs memory` has not been updated. A notes edit alone does not satisfy durable memory closure.
...
The backend Global Memory Agent replaces the old diary-only helper. It is a scheduled/manual app-level structured tool-agent run over completed-turn event manifests after the durable `events.sequence` watermark, not a per-turn project worker. Manifest packing adds completed turns one by one and stops before either 80 turns or the 60k estimated-token cap. The Memory Agent also consumes Socrates-authored memory notes as high-signal leads. Its scoped tool calls update profile/identity or create skill proposals; after tools stop, the shared structured runner requests one strict journal output containing summary, observed patterns, affected skills, decisions, open investigations, and next-run focus. A successful run atomically persists the job plus one journal row and refreshes `~/.Socrates/memory_agent/MEMORY_AGENT_LEDGER.md`. The next wake receives automatic statistics, recent proposal/Writer outcomes, the previous handoff, unresolved investigations, and the latest three summaries. `read_memory_journal` can list at most 10 compact runs or read one bounded historical output; no journal embeddings exist in V1. Processed memory notes close with exactly one outcome: `applied`, `already_represented`, `skipped`, or `proposed_skill`. The memory-agent model call retains the same V1 170k memory-mode context-compression trigger. Failures are logged and must not fail the user chat turn.

User memory opt-outs override normal memory candidacy. Main Socrates must not create `memory_note` entries, project-doc updates, or checkpoint reconciliation from genuinely opted-out content; automatic candidate retrieval and deterministic selection must exclude opted-out content; and the Global Memory Agent must inspect the exact full Q&A parent before any durable action and must not place opted-out content in identity, user profile/evidence anchors, skill proposals, or its durable journal. Intent is semantic rather than keyword-based, so quoted examples, hypotheticals, and feature discussions are not opt-outs by themselves. A clearly scoped instruction excludes only that content; broad or ambiguous scope excludes the entire source user message. If an opted-out source nevertheless reaches the memory-note inbox, the note is closed as `skipped` with a generic resolution that does not repeat the withheld content.

Memory Center (`/memory`) is the global memory-agent UI. It contains agent status, pending signal, model/thinking/cadence settings, run history, global MCP controls, global skill creation/deletion, and read-only memory files. It must remain a fixed-shell professional UI: header and footer do not scroll, the middle region is the scroll surface, and on desktop the left content and right Memory Files rail scroll independently. Core Memory exposes `identity.md`, `user_profile.md`, and the generated Memory Agent Ledger snapshot; `operating_principles.md` is not a file card because those principles are a section inside Identity.
...
The backend must normalize and deduplicate before inserting. Same-turn duplicate notes should return the existing note rather than create another inbox row, and the store must enforce a maximum of two Socrates-authored memory notes per user-turn. After each accepted note, the runtime may expose a tiny "saved memory this turn" ledger to later same-turn continuations, but that ledger belongs near the end of the dynamic turn context. It must not be inserted into the stable system prompt, early prompt prefix, or any cache-sensitive header.
...
mark_done(noteNumber, outcome, resolution)
  closes the item with one of applied, already_represented, skipped, or proposed_skill plus a one-line reason
```
...
Memory Agent classification comes before writes. Durable facts, preferences, allergy/safety boundaries, and global active user context belong in `user_profile.md` with evidence anchors when important. Project-specific active context belongs in `.socrates/PROJECT_NOTES.md` and should be skipped by the global Memory Agent with an explicit resolution. Rare Socrates behavior/identity changes go through the identity confirmation path. Only reusable procedures become skill proposals. Notes whose evidence is already represented in `user_profile.md`, `identity.md`, or an existing skill should be marked `already_represented`; weak, project-local, or current-turn-only leads should be marked `skipped`.

Skill proposal scope is owned by the Memory Agent. Socrates-originated notes carry a backend default of project scope because they came from a project turn; the Memory Agent keeps that scope for most project-local workflows and upgrades to global only when the procedure is clearly reusable across projects. Autonomous Memory Agent skill proposals also choose project or global scope. The backend resolves create versus update from the canonical target path at proposal time; model wording cannot create a duplicate when that scoped skill already exists.
...
Settings exposes five independent worker model selectors for Skill Writer, Socrates Context Compactor, Memory Context Compactor, Title Generator, and Frontier. The main Socrates picker also controls the same-Socrates goal-resolution phase; the Global Memory Agent retains its separate settings surface. Each selector uses the normal model registry and thinking options. Socrates Context Compactor controls shared Classic/Flow 170k conversation compression, while fine-grained within-turn tool-output disposition is chosen by the active main Socrates model without another worker call. Memory Context Compactor controls Global Memory Agent and Skill Writer compression. No Goal Router or Memory Router selector remains. Frontier defaults to OpenRouter `x-ai/grok-4.5` with low reasoning; OpenRouter declares reasoning mandatory for that endpoint, so its selector exposes Low, Medium, and High rather than an invalid Off choice.

Current unified pre-turn and reconciliation flow:

```text
pre-turn
  -> attach Global Always-Apply Rules and Project Always-Apply Rules
  -> start typed goal-card and memory-candidate retrieval concurrently
  -> give the same selected Socrates the exact latest message, current capsule,
     latest exact exchange, and at most five numbered current/older candidates
  -> decide only current, older, new, or clarify with zero tools
  -> bind, reopen, or create the canonical goal deterministically
  -> select authorized exact memory deterministically after binding
  -> prepare one view-neutral exact context and begin main work

genuine finalization
  -> assemble exact task evidence references for the complete request lifecycle
  -> have the same main Socrates judge whether durable `.socrates` changes are needed
  -> have Socrates read, mutate, and re-read each exact target when required
  -> block the final answer until every same-Socrates mutation target is re-read and verified
  -> skip ordinary answers and already-represented facts
```

If same-Socrates resolution remains invalid after one structured repair, the backend retains an available current goal or asks for clarification; it never guesses by keywords or scores. Goal and memory retrieval failures are recorded independently in the typed receipt, failed recall is not claimed as successful, and the safe remaining path may continue. There is no retry queue.
...
`skills` now provides the matching chat lifecycle for portable Agent Skill packages. `preview_import` accepts exactly one exact user-supplied public HTTPS ZIP URL or current-turn chat ZIP attachment path. URL downloads are bounded and SSRF-guarded; attached ZIPs are bounded and verified against the current conversation/turn. Both run the same no-execution ZIP validation/security inspection as the dashboard and return a capped model preview plus destination-bound 24-hour preview id. `commit_import` is approval-required, defaults to project scope and reject-on-conflict, then reuses the same atomic install/rollback path. Socrates must report warnings and verify the installed skill; it cannot use this operation to search for packages or bypass preview through Terminal.
...
Normal search rows are intentionally small: `resultNumber`, `content`, `turnId`, `conversationTitle`, `turnNumber`, `matchedRole`, visible `status`, and `occurredAt`. Scores, conversation/message ids, chunk/vector ids, and storage ids stay internal. `inspect` resolves a returned result or turn reference to its full parent; audit retains its own exact runtime filters.

Ordinal recall uses a queryless lexical search with the literal integer in `turnNo`, preferably narrowed by `conversationTitle` when the conversation is known. It returns the same clean Q&A-parent rows as other trace retrieval, not separate message rows. The backend does not parse ordinal phrases out of `query`; this avoids false positives such as matching "turn 2" against "turn 20". Project-scoped ordinal search may return the same turn number from multiple visible conversations, so Socrates should inspect the relevant numbered result before making precise claims.
...
For non-trivial implementation/build work, Socrates uses `.socrates/` as one bounded agent workspace: `PLAN.md` holds the current plan, `TASKS.md` holds the live checklist, and `work/` holds disposable investigation scripts, test probes, generated diagnostics, and temporary artifacts. Those transient artifacts are not durable memory authorities and do not replace `project_docs` or `repo_docs`. Production code, user deliverables, migrations, and permanent tests belong in the attached workspace/repo. When the user asks Socrates to write code or create a script/program, Socrates should use `edit` to create the production file in a sensible repo location. If the work is based on files in a subfolder, generated outputs should stay in that subfolder or the nearest relevant existing folder unless the user says otherwise. If the location is ambiguous and the user says Socrates can decide, use that nearest relevant folder when one is known; use the repo root only for genuinely project-level or standalone workspace-level work, or a small well-named folder for natural multi-file work.
...
OpenRouter cache-friendly routing uses a stable project/conversation cache-affinity key, sent as top-level `session_id` and `prompt_cache_key` provider options. Socrates no longer leaves multi-provider routing empty: multi-provider OpenRouter models send price-first routing with `sort: "price"` and `allow_fallbacks: true`, while OpenRouter DeepSeek V4 routes use cheap-compatible ranked provider orders. After the first OpenRouter call in a turn reports its actual routed provider, later continuations in that same turn prefer that provider while keeping fallbacks allowed. Deliberate provider pins use OpenRouter provider slugs, not display labels; title-generation routes pin `deepinfra` or `alibaba`. OpenRouter usage metadata is persisted with routed provider, raw usage, cache-read/write tokens, and provider response metadata; when provider cost is absent, Socrates computes a marked estimate from the versioned endpoint-pricing snapshot in `packages/providers`.

Direct DeepSeek API calls use the official `https://api.deepseek.com/chat/completions` route through `packages/providers/src/deepseek`. DeepSeek KV caching is automatic, so Socrates' responsibility is to preserve stable prompt prefixes and normalize provider usage fields: `prompt_cache_hit_tokens` becomes cached input tokens and `prompt_cache_miss_tokens` becomes uncached input tokens. Direct DeepSeek thinking streams use `reasoning_content`; if a thinking call emits tool calls, that reasoning content must be passed back by the DeepSeek adapter during the same turn's tool-loop continuations. Direct DeepSeek also requires every function tool's `parameters` to be a top-level object JSON Schema; the provider layer normalizes model-visible union schemas into object-shaped provider parameters while core keeps strict `packages/contracts` validation for the actual tool call.

The latest direct DeepSeek browser verification on 2026-07-08 used Test-Workspace chat `conv_02148ef15f534408bb587a92cb7eb71e`: direct `deepseek-v4-pro`, High thinking, visible streamed Thinking part, one completed `current_time` tool call, and persisted DeepSeek cache telemetry on the continuation call (`prompt_cache_hit_tokens` 12544, `prompt_cache_miss_tokens` 239). `/api/models` also confirmed direct `deepseek-v4-pro` and `deepseek-v4-flash` rows under `DeepSeek API`, each with Off/High/Max thinking and High default.
...
user sends first message
  ...
  -> for direct DeepSeek thinking+tool continuations in the same turn, preserve assistant reasoning_content in the next provider request
...
The composer is the actual shared Classic `ChatComposer`, preserving the exact model/thinking menus, send/stop behavior, file picker, image paste, image/file preview tray, drag/drop, vision warning, Agent Skill ZIP support, attachment limits, and 10,000-character large-paste attachment rule. Both Classic and Flow now receive the same optional push-to-talk microphone presentation. Classic sends the normalized recording to its temporary conversation-scoped STT endpoint and appends the transcript to its draft without auto-sending or creating V2 state. Flow uses the V2 speech path, and a finalized transcript enters the same unified pre-turn lifecycle as typed text. Neither view adds a separate Tools control. Per-response read aloud remains outside the composer. Users do not create chats or manually organize goal boundaries, though direct switch/pause/finish/reopen/archive/pin controls remain available.

V2 uses the same Socrates agent, tools, providers, workspace `.socrates/`, global `~/.Socrates/`, concurrent candidate retrieval, deterministic memory selection, and global Memory Agent as Classic. Both views use the selected main model through `SocratesAgent.resolveGoal`, the main prompt core, shared runtime, zero tools, and the strict current/older/new/clarify schema. The phase receives at most five whole numbered goal cards and never authors opaque ids. It does not use the Classic conversation-title rewriter or a capsule-writing LLM: deterministic goal titles and materiality-gated rich capsule versions provide the Seamless navigation/resume labels. Canonical task and message identity determines the one authoritative physical source. Detailed turn, tool, approval, Terminal, evidence, context, usage, and event records remain in that source runtime's adapter tables and are projected across views rather than duplicated. Canonical Q&A parents reuse shared LanceDB lexical/semantic/combined retrieval under their physical `runtimeKind` and source scope; queryless/inspect/audit continue to resolve through the authoritative source evidence.
...
Full routing, context, voice, concurrency, recovery, and validation boundaries are in `V2_FLOW_ARCHITECTURE.md`. Whole-repo regression, production builds, focused browser interaction verification, and supported-platform v0.1.19 release archives have passed for the released foundation. Phase 4 was verified on 2026-07-26 with an isolated five-turn real DeepSeek Flow at desktop and mobile widths: one replace-in-place activity sentence, no provisional final, Projects to Goals to Queries navigation, completed-focus persistence, view-only historical selection, and a canonical Flow to Classic to Flow round trip. The detailed evidence is in `evals/flow-convergence/phase-4-real-e2e.md`. The extended soak remains outstanding. The existence of the UI must not be described as proof of 24-hour unattended reliability.
