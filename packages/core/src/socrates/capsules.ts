import type { SocratesCapsuleRefreshReason, SocratesGoal, SocratesGoalCapsule } from "./types"

export const DEFAULT_Socrates_CAPSULE_STALE_AFTER_SEQUENCES = 6
export const MAX_Socrates_CAPSULE_LIST_ITEMS = 12

export type SocratesCapsuleRefreshEvent = Readonly<{
  kind: "goal_created" | "goal_parked" | "material_change" | "pre_compaction" | "goal_completed" | "turn_completed"
  sequence: number
}>

export type SocratesGoalCapsulePatch = Readonly<{
  summary: string
...
export const capsuleRefreshReason = (input: {
  ...
  event: SocratesCapsuleRefreshEvent
  staleAfterSequences?: number
}): SocratesCapsuleRefreshReason | undefined => {
  ...
  const staleAfter = clampInteger(input.staleAfterSequences ?? DEFAULT_Socrates_CAPSULE_STALE_AFTER_SEQUENCES, 1, 10_000)
  return input.event.sequence - input.capsule.sourceThroughSequence >= staleAfter ? "stale" : undefined
}

export const shouldRefreshSocratesGoalCapsule = (input: {
  capsule?: SocratesGoalCapsule
  event: SocratesCapsuleRefreshEvent
  staleAfterSequences?: number
}): boolean => capsuleRefreshReason(input) !== undefined

export const refreshSocratesGoalCapsule = (input: {
  capsuleId: string
  goal: SocratesGoal
  previous?: SocratesGoalCapsule
  patch: SocratesGoalCapsulePatch
  ...
  if (input.previous && input.previous.goalId !== input.goal.id) {
    throw new Error("A Socrates goal capsule can only be refreshed for its own goal.")
  }
...
export const renderSocratesGoalCapsuleForRouting = (capsule: SocratesGoalCapsule): string =>
  [
...
const normalizeList = (values: readonly string[]): string[] =>
  [...new Set(values.map((value) => value.trim()).filter(Boolean))].slice(0, MAX_Socrates_CAPSULE_LIST_ITEMS)
