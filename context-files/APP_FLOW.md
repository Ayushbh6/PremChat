...
This document defines product routes, page responsibilities, and the currently implemented Classic/Flow navigation. `FLOW_NORTH_STAR.md` owns the target product intent and `UNIFIED_SOCRATES_LIFECYCLE.md` owns the detailed target lifecycle/cleanup boundary. `V2_FLOW_ARCHITECTURE.md` records released implementation and migration constraints. Where this current-flow inventory still mentions a post-evidence Memory Router, mutable focus ledger, or view-specific trace adapter, it describes convergence debt rather than accepted target behavior.
