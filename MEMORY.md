...
Before any agent architecture, capability, tool, shared utility, prompt, routing, retrieval, embeddings, vector search, context management, provider, worker-agent, generated tool-documentation, goal/task/capsule lifecycle, access scope, reconciliation, ledger, finalization, or UI-navigation work, read root `AGENTS.md`, `context-files/REPO_RULES.md`, `context-files/FLOW_NORTH_STAR.md`, `context-files/UNIFIED_SOCRATES_LIFECYCLE.md`, `context-files/AGENT_REFACTOR_MANIFESTO.md`, and `context-files/AGENT_CAPABILITY_WORKFLOW.md` completely. The manifesto is the agent-core authority and the workflow is mandatory; phase reports are historical. The target sequence is: persist the exact message; retrieve goal and memory candidates in parallel; same-Socrates decision of current, retrieved older goal, new, or clarify; deterministic exact memory selection; one shared Socrates loop; validated atomic answer/task/goal/capsule commit; publication; asynchronous enrichment. There is no target separate Goal Router agent, goal-search tool loop, model-driven Memory Router, or mutable main-agent ledger. Model-context efficiency is shared runtime policy: release eligible large current-turn tool projections with turn-local `R<n>` handles, and automatically compact the oldest model-visible history at 170k while preserving canonical exact sources and an approximately 70k newest whole-turn suffix.
